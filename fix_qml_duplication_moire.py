...
def check_cylinder_duplication():
    ...
    for pos in positions:
        if pos in position_dict:
            position_dict[pos] += 1
            print(f"⚠️ ДУБЛИРОВАНИЕ ПОЗИЦИИ: {pos} (встречается {position_dict[pos]} раз)")
        else:
            position_dict[pos] = 1
...
def check_scene_environment_usage():
    ...
    effect_props = [
        'bloomEnabled', 'ssaoEnabled', 'tonemapMode', 
        'antialiasingMode', 'shadowsEnabled'
    ]
...
def analyze_antialiasing_values():
    ...
    aa_quality_match = re.search(r'property\s+int\s+antialiasingQuality:\s*(\d+)', content)
...
def suggest_fixes():
    ...
    print("   ❌ Убрать отдельные цилиндры, если они дублируют OptimizedSuspensionCorner")
