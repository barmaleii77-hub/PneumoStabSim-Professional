...
class TestP10Window(QMainWindow):
    ...
    def _update_simulation(self):
        ...
        for i, (line_enum, pressure) in enumerate(
            zip([Line.A1, Line.B1, Line.A2, Line.B2], line_pressures)
        ):
            snapshot.lines[line_enum] = LineState(
                line=line_enum, pressure=max(101325.0, pressure), temperature=293.15
            )
