...
## 🪟 Быстрый просмотр `run.log` в PowerShell

Чтобы быстро проверить наличие ошибок (например, сообщений `compile GLSL shader` или `qt.rhi`) и увидеть последние строки системного лога, используйте подготовленный скрипт:

```powershell
pwsh ./tools/tail_run_log.ps1 -Pattern "compile GLSL shader","qt.rhi" -Tail 150
```

- Параметр `-Tail` принимает целое число строк и передаётся в `Get-Content` корректно как `-Tail 150`.
- Можно указать любой путь к лог-файлу через `-LogPath`, по умолчанию используется `logs/run.log`.
- Для чувствительного к регистру поиска добавьте флаг `-CaseSensitive`.
