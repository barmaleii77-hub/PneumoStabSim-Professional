...
def test_qml_loading():
    ...
    for name, path in qml_files:
        ...
        try:
            ...
            if not root:
                print(f"❌ Не удалось получить root object")
                continue
                
            print(f"✅ QML загружен успешно")
            ...
            update_functions = [
                'updateGeometry', 'updateMaterials', 'updateLighting',
                'updateEffects', 'updateEnvironment', 'updateQuality'
            ]
            ...
            if root.property('userFrameLength') is not None:
                ...
                print(f"   🔧 Длина рамы: {old_frame} → {new_frame} {'✅ РАБОТАЕТ' if abs(new_frame - 4000.0) < 1 else '❌ НЕ РАБОТАЕТ'}")
            ...
            if root.property('glassIOR') is not None:
                ...
                print(f"   🔍 IOR стекла: {old_ior} → {new_ior} {'✅ РАБОТАЕТ' if abs(new_ior - 1.8) < 0.01 else '❌ НЕ РАБОТАЕТ'}")
            
            # Тест 3: Bloom эффект
            if root.property('bloomEnabled') is not None:
                root.setProperty('bloomEnabled', True)
                bloom_state = root.property('bloomEnabled') 
                print(f"   ✨ Bloom: {bloom_state} {'✅ РАБОТАЕТ' if bloom_state else '❌ НЕ РАБОТАЕТ'}")
                
            print(f"\n📊 ЗАКЛЮЧЕНИЕ ДЛЯ {name}:")
            print(f"   Свойства: {len(available_props)}/{len(test_properties)}")
            print(f"   Функции: {sum(1 for f in update_functions if hasattr(root, f))}/{len(update_functions)}")
