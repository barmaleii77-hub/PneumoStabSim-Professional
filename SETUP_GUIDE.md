...
> 💡 Если вы работаете в контейнере или на CI, воспользуйтесь командой
> `make install-qt-runtime`, чтобы автоматически выполнить установку через
> скрипт `tools/install_qt_runtime.sh`. Он проверит наличие `apt-get`, при
> необходимости попросит повышенные права и подтянет те же пакеты и их зависимости.

Эта команда подтягивает весь необходимый стек (`libglx-mesa0`, `mesa-vulkan-drivers`, `libxcb-xkb1` и прочие зависимости), после чего проверка окружения `python -m tools.autonomous_check --launch-trace` и обычный запуск `python app.py --test-mode` проходят без предупреждений даже в headless-контейнере.

> 🛠️ **Qt Shader Baker (`qsb`)**
>
> Для сборки и проверки шейдеров используется утилита `qsb`. В образах CI она
> отсутствует по умолчанию, поэтому перед запуском `make check` установите
> пакет `qt6-shader-baker` (вместе с `qt6-base-dev-tools`, если его ещё нет):
>
> ```bash
> sudo apt-get update
> sudo apt-get install -y qt6-base-dev-tools qt6-shader-baker
> sudo ln -s /usr/lib/qt6/bin/qsb /usr/local/bin/qsb  # добавляем в PATH
> ```
>
> Это гарантирует, что шаг `python3 tools/validate_shaders.py` в `make check`
> завершится успешно без ошибок об отсутствии `qsb`.
