...
def is_suitable_runtime(runtimes, runtime_name, platform, min_version):
    for runtime in runtimes:
        ...
        if (
            (runtime["name"] == runtime_name or identifier == runtime_name)
            and is_available(runtime)
            and identifier.startswith(
                "com.apple.CoreSimulator.SimRuntime.{}".format(platform)
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--minimum-deployment-target", type=StrictVersion, default="0.0"
    )
    parser.add_argument(
        "--state",
        choices=["booted", "shutdown", "creating", "booting", "shutting-down"],
        action="append",
    )
    ...
    for runtime_name in device_dict:
        if is_suitable_runtime(
            runtimes, runtime_name, args.platform, args.minimum_deployment_target
        ):
            for device in device_dict[runtime_name]:
                if is_available(device) and (
                    args.state is None or device["state"].lower() in args.state
                ):
                    print(device["udid"])
