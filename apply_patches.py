...
class PatchApplier:
    ...
    def apply_patch_git(self, patch_file: str, target_file: str) -> bool:
        ...
        try:
            result = subprocess.run(
                check_cmd, capture_output=True, text=True, cwd=str(self.project_root)
            )

            if result.returncode != 0:
                print("⚠️ Патч проверка не пройдена:")
                print(f"   {result.stderr}")
                return False

            print("✅ Патч проверка пройдена")

            # Применяем патч
            apply_cmd = ["git", "apply", str(patch_path)]
            result = subprocess.run(
                apply_cmd, capture_output=True, text=True, cwd=str(self.project_root)
            )
    ...
    def apply_patch_manual(self, patch_file: str, target_file: str) -> bool:
        ...
        try:
            ...
            print("✅ Патч применен (упрощенный метод)")
            print("⚠️ Рекомендуется проверить изменения вручную")
