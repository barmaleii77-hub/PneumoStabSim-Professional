...
class ComparisonReport:
    ...
    def print_report(self):
        ...
        if (
            len(self.matched_widgets) == len(self.monolith_widgets)
            and len(self.monolith_widgets) > 0
        ):
            print("\n✅ ВСЕ ПАРАМЕТРЫ МОНОЛИТА ПРИСУТСТВУЮТ В РЕФАКТОРИНГЕ!")
        else:
            coverage = (
                len(self.matched_widgets) / len(self.monolith_widgets) * 100
                if self.monolith_widgets
                else 0
            )
...
def extract_labeled_sliders(file_path: Path) -> List[WidgetUsage]:
    ...
    pattern_old = re.compile(
        r'(?<!self\.)(\w+)\s*=\s*LabeledSlider\(\s*["\']([^"\']+)["\']', re.MULTILINE
    )
...
def generate_migration_plan(report: ComparisonReport, monolith_path: Path):
    ...
    for param in report.missing_in_refactored:
        ...
        widget = next(
            (w for w in report.monolith_widgets if w.parameter == param), None
        )
        ...
        for tab, keywords in categorization.items():
            if any(kw in param_lower for kw in keywords):
                if widget:
                    ...
                    for i in range(
                        start_line, min(start_line + 5, len(monolith_lines))
                    ):
                    ...
                    tab_assignments[tab].append(
                        {"param": param, "line": 0, "code": f"# TODO: Добавить {param}"}
                    )
                assigned = True
                break
        ...
        if not assigned and widget:
            ...
            print(
                f"   ⚠️  Не удалось определить таб для: {param} (строка {widget.line_number})"
            )
...
def main():
    ...
    for panel_config in panels:
        ...
        if not panel_config["refactored_dir"].exists():
            print(
                f"⚠️  Директория рефакторинга не найдена: {panel_config['refactored_dir']}"
            )
            continue
    ...
    for report in all_reports:
        if len(report.monolith_widgets) > 0:
            ...
            print(
                f"   {status} {report.panel_name}: {coverage:.1f}% ({len(report.matched_widgets)}/{len(report.monolith_widgets)})"
            )
    ...
    ready_panels = [
        ...
        if len(r.missing_in_refactored) == 0 and len(r.monolith_widgets) > 0
    ]
    ...
    if incomplete_panels:
        ...
        for r in incomplete_panels:
            print(
                f"   • {r.panel_name}: добавить {len(r.missing_in_refactored)} параметров"
            )
