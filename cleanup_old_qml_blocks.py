...
def remove_old_materials(content: str) -> str:
    ...
    matches = list(
        re.finditer(material_pattern, content, flags=re.MULTILINE | re.DOTALL)
    )
    ...
    for match in matches:
        ...
        if (
            "SharedMaterials {" in before_match
            and "}" not in before_match[before_match.rfind("SharedMaterials {") :]
        ):
...
def remove_old_lights(content: str) -> str:
    ...
    matches = list(
        re.finditer(dir_light_pattern, content, flags=re.MULTILINE | re.DOTALL)
    )
    ...
    for match in matches:
        ...
        before_match = content[: match.start()]
        if (
            "DirectionalLights {" in before_match
            and "}" not in before_match[before_match.rfind("DirectionalLights {") :]
        ):
    ...
    matches = list(
        re.finditer(point_light_pattern, content, flags=re.MULTILINE | re.DOTALL)
    )
    ...
    for match in matches:
        ...
        before_match = content[: match.start()]
        if (
            "PointLights {" in before_match
            and "}" not in before_match[before_match.rfind("PointLights {") :]
        ):
            continue
