...
def analyze_qml_file(qml_path):
    ...
    component_decl_matches = re.findall(component_declarations_pattern, content, re.MULTILINE)
    ...
    duplicated_positions = {pos: count for pos, count in position_count.items() if count > 1}
    ...
    duplicated_scales = {scale: count for scale, count in scale_count.items() if count > 1}
    ...
    # Анализ материалов
    print("\n🎨 АНАЛИЗ МАТЕРИАЛОВ:")
    material_pattern = r'materials:\s*PrincipledMaterial\s*\{[^}]*baseColor:\s*"([^"]+)"'
    ...
    for color in joint_colors:
        if material_count.get(color, 0) > 4:
            print(f"  ⚠️  Подозрение на дублирование шарниров цвета {color}: {material_count[color]} экземпляров (ожидается 4)")
            issues_found += 1
    
    # Проблема 2: Слишком много цилиндров
    if geometry_count.get("#Cylinder", 0) > 20:
        print(f"  ⚠️  Очень много цилиндров: {geometry_count.get('#Cylinder', 0)} (может быть дублирование)")
        issues_found += 1
    ...
    for comp_name, expected_count in expected_instances.items():
        ...
        if actual_count != expected_count:
            if expected_count == 0 and actual_count > 0:
                print(f"  ⚠️  Найдены устаревшие компоненты {comp_name}: {actual_count} (должно быть {expected_count})")
                issues_found += 1
            elif expected_count > 0 and actual_count != expected_count:
                print(f"  ⚠️  Неправильное количество {comp_name}: {actual_count} (ожидается {expected_count})")
                issues_found += 1
        else:
