...
1. Скопируйте шаблон окружения: `cp env.sample .env` и обновите пути к Qt при необходимости.
2. Запустите автоматический скрипт: `python setup_environment.py --python-version 3.13 --install-qt --qt-version 6.7.2 --hash-file requirements.txt`.
3. При повторных запусках используйте `source activate_environment.sh --python-version 3.13` (Linux/macOS) или `./activate_environment.ps1 -PythonVersion 3.13` (Windows).

📘 Подробная пошаговая инструкция доступна в разделе [docs/ENVIRONMENT_SETUP.md](docs/ENVIRONMENT_SETUP.md)

### 1. Требования
- **Python 3.13** (протестировано на 3.13.0)
...
2. **Откройте файл `PneumoStabSim.code-workspace`**: в нем уже прописан кроссплатформенный путь к виртуальному окружению `.venv` и fallback на системный `python3.13`.
3. **Проверьте выбор интерпретатора**: команда `Python: Select Interpreter` покажет `.venv` для Windows (`.venv\Scripts\python.exe`) и Unix (`.venv/bin/python`). Если окружение еще не создано – Insiders автоматически использует `python3.13`.
