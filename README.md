...
python app.py --safe              # Headless safe mode (без QML сцены)
...
### 🛰️ Headless / CI режимы

Автоматические тесты и CI-сборки могут выполняться без доступного дисплея. Для
устойчивого запуска используйте следующие настройки:

```sh
export QT_QPA_PLATFORM=offscreen        # Отключает требования к X11/Wayland
python app.py --safe                    # Не создаёт окно и завершает Qt сразу после инициализации

# При необходимости отката на Qt Widgets без Qt Quick 3D:
python app.py --legacy --safe-mode      # Legacy UI + авто выбор backend
```

Флаг `--safe` автоматически деактивирует Qt Quick 3D сцену, переключает
приложение в «безоконный» режим и завершает процесс сразу после успешного
создания `QApplication`. Для диагностических прогонов можно дополнительно
использовать `--test-mode`, чтобы окно (если оно создано) закрылось через 5
секунд.
