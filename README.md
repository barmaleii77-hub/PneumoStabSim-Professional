...
1. Убедитесь, что у вас установлен Python 3.12 или 3.13 и доступна утилита `make`.
...
### 1. Требования
- **Python 3.12–3.13** (протестировано на 3.13.0)
...
2. **Откройте файл `PneumoStabSim.code-workspace`**: в нем уже прописан кроссплатформенный путь к виртуальному окружению `.venv` и fallback на системный `python3.12`/`python3.13`.
3. **Проверьте выбор интерпретатора**: команда `Python: Select Interpreter` покажет `.venv` для Windows (`.venv\Scripts\python.exe`) и Unix (`.venv/bin/python`). Если окружение еще не создано – Insiders автоматически использует `python3.12` или `python3.13`.
