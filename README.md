...
### 🧯 Безопасные и совместимые режимы запуска

- `--safe` и `--test-mode` активируют один и тот же сценарий: Qt не загружает
...
- Комбинируйте `--legacy` + `--safe`, чтобы проверить рендер-конфигурацию в
  headless-прогоне без необходимости поднимать полноценную сцену.
