...
1. Подготовьте зависимости для активной платформы (Linux или Windows):

   ```sh
   make cross-platform-prep
   ```

   Скрипт `tools/cross_platform_test_prep.py` установит системные библиотеки
   для Qt, синхронизирует Python-зависимости (`uv sync --extra dev`) и при
   необходимости скачает Qt SDK.

2. Запустите полный тестовый набор с headless-профилями Qt Quick:

   ```sh
   make cross-platform-test
   ```

   Команда принудительно активирует `QT_QPA_PLATFORM=offscreen` и другие
   значения, чтобы тесты (включая QML/Qt Quick) выполнялись и на Linux,
   и на Windows без пропусков.

3. Повторите шаги 1–2 на второй ОС (например, в отдельном CI-агенте или на
   рабочей станции Windows), чтобы подтвердить полный кроссплатформенный прогон.

> Если требуется тонкая настройка (например, пропустить установку системных
> пакетов или передать дополнительные аргументы pytest), используйте
> `python -m tools.cross_platform_test_prep --help`.
...
**🎯 PneumoStabSim Professional v4.9.8** - Готов к профессиональному использованию
