...
> 💡 **CI и бездисплейные среды.** Для прогонов в headless-режиме установите
> `QT_QPA_PLATFORM=offscreen` (при необходимости добавьте
> `QSG_RHI_BACKEND=opengl`) и запустите `python app.py --safe` или
> `python app.py --test-mode`. Оба флага теперь используют единый безопасный
> сценарий: Qt не загружает `main.qml`, сцена Qt Quick 3D не создаётся, а
> завершение инициируется через `QCoreApplication.exit(0)` сразу после
> инициализации. При необходимости сочетайте с `--legacy`, чтобы активировать
> чистый OpenGL-виджет без зависимости от современного GPU.
