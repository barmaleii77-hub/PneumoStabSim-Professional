...
python app.py --safe              # Headless safe-режим без Qt Quick 3D
...
> `QT_QPA_PLATFORM=offscreen` и запустите `python app.py --safe --legacy`.
> Флаг `--safe` отключает создание QML-сцены и завершает приложение сразу после
> инициализации, а `--legacy` активирует виджет на чистом OpenGL. В CI этот
> режим гарантирует отсутствие обращения к GPU и обеспечивает корректный код
> выхода при ошибках физической модели.
