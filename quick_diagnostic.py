...
def check_system_resources():
    ...
    print(
        f"Память: {memory.total // (1024**3)} GB всего, {memory.available // (1024**3)} GB доступно ({memory.percent}% используется)"
    )

    # Диск
    disk = psutil.disk_usage(".")
    print(
        f"Диск: {disk.total // (1024**3)} GB всего, {disk.free // (1024**3)} GB свободно"
    )
...
def check_python_environment():
    ...
    packages_to_check = ["PySide6", "psutil", "numpy", "logging"]
    ...
    if python_processes:
        ...
        for proc in python_processes[:5]:  # Показываем первые 5
            try:
                ...
                print(
                    f"  PID {proc.info['pid']}: CPU {proc.info['cpu_percent']}%, Память {memory_mb:.1f}MB"
                )
            except:
