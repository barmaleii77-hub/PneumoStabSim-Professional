...
class PerformanceMonitor:
    ...
    def get_average_metrics(self, last_n: int = 10) -> Dict[str, float]:
        ...
        return {
            ...
            'avg_fps': sum(m.fps for m in recent_metrics if m.fps) / max(sum(1 for m in recent_metrics if m.fps), 1),
            'avg_frame_time_ms': sum(m.frame_time_ms for m in recent_metrics if m.frame_time_ms) / max(sum(1 for m in recent_metrics if m.frame_time_ms), 1)
        }
    ...
    def print_status(self):
        ...
        print("=" * 50)
        print(f"CPU: {current.cpu_percent:.1f}% (среднее: {averages.get('avg_cpu_percent', 0):.1f}%)")
        ...
        if current.frame_time_ms:
            print(f"Время кадра: {current.frame_time_ms:.2f}ms (среднее: {averages.get('avg_frame_time_ms', 0):.2f}ms)")
