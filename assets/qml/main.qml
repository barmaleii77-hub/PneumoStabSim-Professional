...
function requireStringSetting(name, value, allowEmpty) {
 ...
 property color backgroundColor: requireStringSetting('startBackgroundColor', startBackgroundColor, false)
 property int backgroundMode: backgroundModeFromString(requireStringSetting('startBackgroundMode', startBackgroundMode, false))
 property bool iblBackgroundEnabled: requireBooleanSetting('startSkyboxEnabled', startSkyboxEnabled)
 property bool iblLightingEnabled: requireBooleanSetting('startIblEnabled', startIblEnabled)
 property string iblPrimarySourceSetting: normalizeSourcePath(requireStringSetting('startIblSource', startIblSource, true))
 property url iblPrimarySourceUrl: resolveIblUrl(iblPrimarySourceSetting)
 property string iblFallbackSourceSetting: normalizeSourcePath(requireStringSetting('startIblFallback', startIblFallback, true))
 property url iblFallbackSourceUrl: resolveIblUrl(iblFallbackSourceSetting)
 property real iblRotationDeg: requireNumericSetting('startIblRotation', startIblRotation, -1080.0,1080.0)
 property bool iblBindToCamera: requireBooleanSetting('startIblBindToCamera', startIblBindToCamera)
 property real iblOffsetXDeg: requireNumericSetting('startIblOffsetX', startIblOffsetX, -180.0,180.0)
 property real iblOffsetYDeg: requireNumericSetting('startIblOffsetY', startIblOffsetY, -180.0,180.0)
 property real iblIntensity: requireNumericSetting('startIblIntensity', startIblIntensity,0.0,8.0)
 property real envProbeBrightness: requireNumericSetting('startProbeBrightness', startProbeBrightness,0.0,8.0)
 property real envProbeHorizon: requireNumericSetting('startProbeHorizon', startProbeHorizon, -1.0,1.0)
 property real envSkyboxBlurAmount: requireNumericSetting('startSkyboxBlur', startSkyboxBlur,0.0,1.0)

 // Fog
 property bool fogEnabledSetting: requireBooleanSetting('startFogEnabled', startFogEnabled)
 property color fogColorSetting: requireStringSetting('startFogColor', startFogColor, false)
 property real fogDensitySetting: requireNumericSetting('startFogDensity', startFogDensity,0.0,1.0)
 property real fogNearSetting: requireNumericSetting('startFogNear', startFogNear,0.0,200000.0)
 property real fogFarSetting: (function() {
 var clamped = requireNumericSetting('startFogFar', startFogFar,500.0,400000.0);
 return clamped < fogNearSetting ? fogNearSetting : clamped;
 })()
 property bool fogHeightEnabledSetting: requireBooleanSetting('startFogHeightEnabled', startFogHeightEnabled)
 property real fogGroundLevelSetting: requireNumericSetting('startFogLeastY', startFogLeastY, -100000.0,100000.0)
 property real fogHeightSetting: requireNumericSetting('startFogMostY', startFogMostY, -100000.0,100000.0)
 property real fogHeightFalloffSetting: requireNumericSetting('startFogHeightCurve', startFogHeightCurve,0.0,4.0)
 property bool fogTransmittanceEnabledSetting: requireBooleanSetting('startFogTransmitEnabled', startFogTransmitEnabled)
 property real fogTransmittanceFalloffSetting: requireNumericSetting('startFogTransmitCurve', startFogTransmitCurve,0.0,4.0)

 // AO
 property bool aoEnabledSetting: requireBooleanSetting('startAoEnabled', startAoEnabled)
 property real aoStrengthSetting: requireNumericSetting('startAoStrength', startAoStrength,0.0,100.0)
 property real aoRadiusSetting: requireNumericSetting('startAoRadius', startAoRadius,0.5,50.0)
 property real aoSoftnessSetting: requireNumericSetting('startAoSoftness', startAoSoftness,0.0,50.0)
 property bool aoDitherSetting: requireBooleanSetting('startAoDither', startAoDither)
 property int aoSampleRateSetting: Math.max(1, Math.min(4, Math.round(requireNumericSetting('startAoSampleRate', startAoSampleRate,1.0,4.0))))
