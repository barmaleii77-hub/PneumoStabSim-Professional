...
Node {
    ...
    function _updatePressureStats() {
        ...
        if (!isFinite(minValue) || !isFinite(maxValue)) {
            ...
            pressureMin = minValue
            pressureMax = maxValue
        }
    }
    ...
    function _linePressure(key) {
        ...
        if (receiver && receiver.pressures && receiver.pressures[key] !== undefined) {
            var direct = Number(receiver.pressures[key])
            if (isFinite(direct))
                return direct
        }
        var entry = _lineEntry(key)
        if (entry && entry.pressure !== undefined) {
            var fallback = Number(entry.pressure)
    ...
    function _pressureRatio(key) {
        ...
        if (!(maxValue > minValue)) {
            return maxValue > 0 ? Math.min(Math.max(value / maxValue, 0.0), 1.0) : 0.0
        }
    ...
    Model {
        ...
        materials: PrincipledMaterial {
            id: matA1
            baseColor: root._sphereColor("a1")
            roughness: 0.25
            metalness: 0.05
            emissiveColor: root._sphereColor("a1")
            emissiveFactor: root._sphereEmissive("a1")
            opacity: 0.85
    ...
    Model {
        ...
        visible: root.hasPressureRange
        materials: PrincipledMaterial {
            baseColor: root._sphereColor("b1")
            roughness: 0.25
            metalness: 0.05
            emissiveColor: root._sphereColor("b1")
            emissiveFactor: root._sphereEmissive("b1")
            opacity: 0.85
            alphaMode: PrincipledMaterial.AlphaBlend
        }
    }
    ...
    Model {
        ...
        visible: root.hasPressureRange
        materials: PrincipledMaterial {
            baseColor: root._sphereColor("a2")
            roughness: 0.25
            metalness: 0.05
            emissiveColor: root._sphereColor("a2")
            emissiveFactor: root._sphereEmissive("a2")
            opacity: 0.85
            alphaMode: PrincipledMaterial.AlphaBlend
        }
    }
    ...
    Model {
        ...
        visible: root.hasPressureRange
        materials: PrincipledMaterial {
            baseColor: root._sphereColor("b2")
            roughness: 0.25
            metalness: 0.05
            emissiveColor: root._sphereColor("b2")
            emissiveFactor: root._sphereEmissive("b2")
            opacity: 0.85
            alphaMode: PrincipledMaterial.AlphaBlend
        }
    }
