...
/*
 * IblProbeLoader — загрузчик HDR для IBL/Skybox.
 *
 * Поддерживает пользовательские HDR-файлы, выбранные через FileCyclerWidget в
 * панели графики: путь поступает из настроек и может указывать как на
 * `file:///` URL, так и на относительный путь внутри проекта. Если файл
 * отсутствует, `FileCyclerWidget` подсветит предупреждение и сохранит значение
 * для диалога; этот компонент логирует переход на fallback и продолжает
 * работу с процедурной HDR-заглушкой, чтобы сцена оставалась освещённой.
 */
Item {
    id: controller
