...
def run_log_diagnostics() -> None:
    ...
    try:
        ...
        diagnostics_ok = (
            ...
            else bool(diag_result.get("ok", True))
        )
...
def _print_sync_discrepancies(quick_diagnostics) -> None:
    ...
    try:
        ...
        if (
            events_sync is not None
            and graphics_sync is not None
            and abs(events_sync - graphics_sync) >= 5.0
        ):
            print("\n—— Несоответствия анализа ————————")
            print(
                f"EVENTS sync_rate: {events_sync:.1f}% vs GRAPHICS sync_rate: {graphics_sync:.1f}%"
            )
...
def _analyze_event_logger() -> None:
    ...
    try:
        ...
        if total > 0:
            ...
            if missing > 0:
                print("   ⚠️  Обнаружены несинхронизированные события!")
            else:
                print("   ✅ Все события успешно синхронизированы")
        else:
            print("   ℹ️  Сигналов не обнаружено")
        ...
        print("   ⚠️  EventLogger не доступен")
    except Exception as e:
