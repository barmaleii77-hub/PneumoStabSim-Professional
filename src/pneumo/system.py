...
class PneumaticSystem:
    ...
    def _validate_system_configuration(self):
        ...
        if actual_wheels != expected_wheels:
            raise ConnectionError(f"Must have cylinders for all wheels. Expected {expected_wheels}, got {actual_wheels}")
        
        # Check we have all 4 lines
        expected_lines = {Line.A1, Line.B1, Line.A2, Line.B2}
        actual_lines = set(self.lines.keys())
        if actual_lines != expected_lines:
            raise ConnectionError(f"Must have all 4 lines. Expected {expected_lines}, got {actual_lines}")
    ...
    def validate_invariants(self) -> ValidationResult:
        ...
        for wheel, cylinder in self.cylinders.items():
            ...
            if not cyl_result['is_valid']:
                errors.extend([f"Cylinder {wheel.value}: {err}" for err in cyl_result['errors']])
            warnings.extend([f"Cylinder {wheel.value}: {warn}" for warn in cyl_result['warnings']])
        
        # Validate each line
        for line_name, line in self.lines.items():
            line_result = line.validate_invariants()
            if not line_result['is_valid']:
                errors.extend([f"Line {line_name.value}: {err}" for err in line_result['errors']])
            warnings.extend([f"Line {line_name.value}: {warn}" for warn in line_result['warnings']])
        ...
        for line_name, volume_info in line_volumes.items():
            if volume_info['total_volume'] <= 0:
                errors.append(f"Line {line_name.value} has zero or negative total volume")
