...
class CylinderState:
    ...
    def __post_init__(self):
        ...
        if abs(self.x) > max_travel / 2.0:
            raise GeometryError(f"Initial position x={self.x} exceeds travel limit +/-{max_travel/2.0}")
    ...
    def update_from_lever_angle(self, angle_rad: float):
        ...
        nominal_length = geom.project_to_cyl_axis(
            tail_3d, 
            (0.0, lever.rod_joint_frac * lever.L_lever, geom.Z_axle)
        )
    ...
    def validate_invariants(self) -> ValidationResult:
        ...
        max_travel = self.spec.geometry.L_travel_max
        if abs(self.x) > max_travel / 2.0:
            errors.append(f"Position x={self.x} exceeds travel limit +/-{max_travel/2.0}")
        ...
        if vol_head < min_vol_head * 0.999:  # Small tolerance for floating point
            errors.append(f"Head volume {vol_head:.6f} below minimum {min_vol_head:.6f}")
