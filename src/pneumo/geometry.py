...
class LeverGeom:
    ...
    def _validate_parameters(self):
        ...
        if self.L_lever <= 0:
            raise GeometryError(f"Lever length L_lever must be positive, got {self.L_lever}")
            
        if not (0.1 <= self.rod_joint_frac <= 0.9):
            raise GeometryError(f"Rod joint fraction must be in [0.1, 0.9], got {self.rod_joint_frac}")
            
        if self.d_frame_to_lever_hinge <= 0:
            raise GeometryError(f"Frame-to-hinge distance must be positive, got {self.d_frame_to_lever_hinge}")
...
class CylinderGeom:
    ...
    def _validate_parameters(self):
        ...
        max_rod_diameter = min(self.D_in_front, self.D_in_rear) * 0.8  # Leave 20% margin
        if self.D_rod >= max_rod_diameter:
            raise GeometryError(f"Rod diameter {self.D_rod} too large for cylinder bores")
        
        # Residual fraction
        if not (0.001 <= self.residual_frac_min <= 0.1):
            raise GeometryError(f"Residual fraction must be in [0.001, 0.1], got {self.residual_frac_min}")
        
        # Travel validation
        L_travel_max = self.L_inner - (self.L_dead_head + self.L_dead_rod + self.t_piston)
        if L_travel_max <= 0:
            raise GeometryError(f"No travel available: L_inner={self.L_inner}, dead zones + piston={self.L_dead_head + self.L_dead_rod + self.t_piston}")
    ...
    def project_to_cyl_axis(self, tail_point: Tuple[float, float, float], 
                           joint_point: Tuple[float, float, float]) -> float:
    ...
    def validate_invariants(self) -> ValidationResult:
        ...
        for is_front in [True, False]:
            ...
            vol_head_max = self.area_head(is_front) * (self.L_inner/2.0 - half_travel - self.L_dead_head)
            vol_rod_max = self.area_rod(is_front) * (self.L_inner/2.0 + half_travel - self.L_dead_rod)
            
            # Volume at maximum compression (x = -half_travel)  
            vol_head_min = self.area_head(is_front) * (self.L_inner/2.0 + half_travel - self.L_dead_head)
            vol_rod_min = self.area_rod(is_front) * (self.L_inner/2.0 - half_travel - self.L_dead_rod)
            
            min_vol_head = self.min_volume_head(is_front)
            min_vol_rod = self.min_volume_rod(is_front)
            
            if vol_head_max < min_vol_head:
                errors.append(f"{label} head volume at max extension below minimum: {vol_head_max:.6f} < {min_vol_head:.6f}")
            if vol_head_min < min_vol_head:
                errors.append(f"{label} head volume at max compression below minimum: {vol_head_min:.6f} < {min_vol_head:.6f}")
            if vol_rod_max < min_vol_rod:
                errors.append(f"{label} rod volume at max extension below minimum: {vol_rod_max:.6f} < {min_vol_rod:.6f}")
            if vol_rod_min < min_vol_rod:
                errors.append(f"{label} rod volume at max compression below minimum: {vol_rod_min:.6f} < {min_vol_rod:.6f}")
