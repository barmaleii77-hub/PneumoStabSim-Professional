...
def mass_flow_incompressible(
    p_up: float, T_up: float, p_down: float, d_eq: float, C_d: float = 0.7
) -> float:
...
def mass_flow_choked(
    p_up: float, T_up: float, d_eq: float, C_d: float = 0.7, gamma: float = GAMMA_AIR
) -> float:
