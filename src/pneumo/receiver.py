...
class ReceiverSpec:
    ...
    V_min: float  # Minimum volume (m?)
    V_max: float  # Maximum volume (m?)

    def __post_init__(self):
        if self.V_min <= 0:
            raise ModelConfigError(f"Minimum volume must be positive, got {self.V_min}")
        if self.V_max <= self.V_min:
            raise ModelConfigError(f"Maximum volume {self.V_max} must be greater than minimum {self.V_min}")
    ...
    def validate_invariants(self) -> ValidationResult:
        ...
        if self.V_min < 1e-6:  # < 1 cm?
            warnings.append(f"Very small minimum volume: {self.V_min} m?")
        if self.V_max > 1.0:  # > 1000 L
            warnings.append(f"Very large maximum volume: {self.V_max} m?")
...
class ReceiverState:
    ...
    def _validate_state(self):
        ...
        if not (self.spec.V_min <= self.V <= self.spec.V_max):
            raise ModelConfigError(f"Volume {self.V} outside valid range [{self.spec.V_min}, {self.spec.V_max}]")
        if self.p <= 0:
    ...
    def apply_instant_volume_change(self, new_V: float):
        ...
        if not (self.spec.V_min <= new_V <= self.spec.V_max):
            raise ThermoError(f"New volume {new_V} outside valid range [{self.spec.V_min}, {self.spec.V_max}]")
        ...
        if self.mode == ReceiverVolumeMode.NO_RECALC:
            ...
            # Adiabatic process: p*V^? = const, T*V^(?-1) = const
            if abs(old_V) < 1e-12:
                raise ThermoError("Cannot perform adiabatic recalculation from zero volume")
    ...
    def validate_invariants(self) -> ValidationResult:
        ...
        if self.T < 200 or self.T > 400:  # Outside -73°C to 127°C
