...
class PneumoLine:
    ...
    name: Line
    endpoints: Tuple[Tuple[Wheel, Port], Tuple[Wheel, Port]]  # ((wheel1, port1), (wheel2, port2))
    ...
    def _validate_configuration(self):
        ...
        if len(self.endpoints) != 2:
            raise ValueError(f"Line must have exactly 2 endpoints, got {len(self.endpoints)}")
        
        # Ensure endpoints are properly formed
        for i, endpoint in enumerate(self.endpoints):
            if len(endpoint) != 2:
                raise ValueError(f"Endpoint {i} must be (Wheel, Port) tuple, got {endpoint}")
            wheel, port = endpoint
            if not isinstance(wheel, Wheel):
                raise ValueError(f"Endpoint {i} wheel must be Wheel enum, got {type(wheel)}")
            if not isinstance(port, Port):
                raise ValueError(f"Endpoint {i} port must be Port enum, got {type(port)}")
    ...
    def validate_invariants(self) -> ValidationResult:
        ...
        if not self.is_diagonal_connection():
            warnings.append(f"Line {self.name.value} is not a diagonal connection: {self.get_connection_description()}")
