...
def advance_gas(dt: float, system: PneumaticSystem, net: GasNetwork, 
                thermo_mode: ThermoMode, log: Optional[logging.Logger] = None):
    ...
    if log:
        for line_name, line_state in net.lines.items():
            log.debug(f"  {line_name.value}: V={line_state.V_curr:.6f}m?, p={line_state.p:.0f}Pa, T={line_state.T:.1f}K")
    ...
    if log:
        ...
        log.debug(f"  Tank: V={net.tank.V:.6f}m?, p={net.tank.p:.0f}Pa, T={net.tank.T:.1f}K, m={net.tank.m:.6f}kg")
        for line_name, line_state in net.lines.items():
            log.debug(f"  {line_name.value}: p={line_state.p:.0f}Pa, T={line_state.T:.1f}K, m={line_state.m:.6f}kg")


def run_gas_simulation(total_time: float, dt: float, system: PneumaticSystem, 
                      net: GasNetwork, thermo_mode: ThermoMode,
                      log_interval: Optional[float] = None,
                      log: Optional[logging.Logger] = None) -> Dict[str, list]:
    ...
    if log:
        log.info(f"Starting gas simulation: T={total_time}s, dt={dt}s, steps={int(total_time/dt)}")
    ...
    while current_time < total_time:
        ...
        # Advance gas state
        advance_gas(actual_dt, system, net, thermo_mode, 
                   log if (log_interval is None or current_time - last_log_time >= log_interval) else None)
        ...
        # Periodic logging
        if log and log_interval is not None and (current_time - last_log_time) >= log_interval:
            log.info(f"t={current_time:.3f}s: Tank p={net.tank.p:.0f}Pa, Lines p=[{', '.join(f'{line.p:.0f}' for line in net.lines.values())}]Pa")
            last_log_time = current_time
    
    if log:
        log.info(f"Gas simulation completed: {step_count} steps, final_time={current_time:.3f}s")
