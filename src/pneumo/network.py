...
from .enums import Line, Wheel, Port, CheckValveKind, ReliefValveKind, ThermoMode
...
class GasNetwork:
    ...
    def __post_init__(self):
        ...
        if actual_lines != expected_lines:
            raise ValueError(f"Network must have all 4 lines. Expected {expected_lines}, got {actual_lines}")
    ...
    def apply_valves_and_flows(self, dt: float, log: Optional[logging.Logger] = None):
        ...
        for line_name, line_state in self.lines.items():
            ...
            if cv_atmo.is_open(PA_ATM, line_state.p):
                ...
                if log:
                    log.debug(f"Atmo->Line {line_name.value}: +{mass_added:.6f}kg, m_dot={m_dot_atmo:.6f}kg/s")
            ...
            if cv_tank.is_open(line_state.p, self.tank.p):
                m_dot_tank = mass_flow_orifice(
                    line_state.p, line_state.T,
                    self.tank.p, self.tank.T,
                    cv_tank.d_eq
                )
                
                # Transfer mass from line to tank
                mass_transferred = min(m_dot_tank * dt, line_state.m * 0.99)  # Prevent complete depletion
                self._transfer_mass_line_to_tank(line_state, mass_transferred)
                
                if log:
                    log.debug(f"Line->Tank {line_name.value}: -{mass_transferred:.6f}kg, m_dot={m_dot_tank:.6f}kg/s")
        
        # Process receiver relief valves
        self._apply_receiver_relief_valves(dt, log)
    
    def _add_mass_to_line(self, line_state: LineGasState, mass_added: float, T_inlet: float):
        ...
        if new_total_mass > 0:
            line_state.T = (old_mass * line_state.T + mass_added * T_inlet) / new_total_mass
    ...
    def _transfer_mass_line_to_tank(self, line_state: LineGasState, mass_transferred: float):
        ...
        if new_tank_mass > 0:
            self.tank.T = (old_tank_mass * self.tank.T + mass_transferred * line_state.T) / new_tank_mass
    ...
    def _apply_receiver_relief_valves(self, dt: float, log: Optional[logging.Logger] = None):
        ...
        if self.tank.p > p_min_threshold:
            ...
            if log:
                log.debug(f"MIN_PRESS relief: -{mass_out_min:.6f}kg, m_dot={m_dot_min:.6f}kg/s")
        ...
        if self.tank.p > p_stiff_threshold:
            ...
            if log:
                log.debug(f"STIFFNESS relief: -{mass_out_stiff:.6f}kg, m_dot={m_dot_stiff:.6f}kg/s")
        ...
        if self.tank.p > p_safety_threshold:
            ...
            if log:
                log.debug(f"SAFETY relief: -{mass_out_safety:.6f}kg, m_dot={m_dot_safety:.6f}kg/s")
    ...
    def enforce_master_isolation(self, log: Optional[logging.Logger] = None):
        ...
        if log:
            log.debug(f"Master isolation: equalized p={equalized_pressure:.0f}Pa, T={avg_temperature:.1f}K")
    ...
    def validate_invariants(self) -> Dict[str, any]:
        ...
        for line_name, line_state in self.lines.items():
            if line_state.m < 0:
                errors.append(f"Line {line_name.value} has negative mass: {line_state.m}")
            if line_state.T <= 0:
                errors.append(f"Line {line_name.value} has non-positive temperature: {line_state.T}")
            if line_state.p < 0:
                errors.append(f"Line {line_name.value} has negative pressure: {line_state.p}")
            if line_state.V_curr <= 0:
                errors.append(f"Line {line_name.value} has non-positive volume: {line_state.V_curr}")
        ...
        if self.master_isolation_open:
            ...
            if len(set(f"{p:.0f}" for p in pressures)) > 1:  # Round to Pa for comparison
                max_p_diff = max(pressures) - min(pressures)
                if max_p_diff > 1000:  # > 1000 Pa difference
                    warnings.append(f"Master isolation enabled but pressures not equalized: diff={max_p_diff:.0f}Pa")
