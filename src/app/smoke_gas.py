...
def create_test_system_and_network(logger):
    ...
    try:
        ...
        logger.info(f"  - Tank: V={gas_network.tank.V:.6f}m?, p={gas_network.tank.p:.0f}Pa")
...
def simulate_kinematic_scenario(system, gas_network, logger):
    ...
    try:
        ...
        for line_name, volume in volumes.items():
            ...
            logger.info(f"  {line_name.value}: V={volume:.6f}m?, p={gas_state.p:.0f}Pa, m={gas_state.m:.6f}kg")
        ...
        for line_name, volume in new_volumes.items():
            ...
            logger.info(f"  {line_name.value}: V={volume:.6f}m? ({vol_change:+.2f}%), p={gas_state.p:.0f}Pa")
...
def run_gas_time_simulation(system, gas_network, logger):
    ...
    try:
        ...
        for line_name in gas_network.lines.keys():
            ...
            min_p = min(pressures)
            max_p = max(pressures)
            logger.info(f"  {line_name.value}: {min_p:.0f} - {max_p:.0f}Pa (range: {max_p-min_p:.0f}Pa)")
...
def validate_final_state(gas_network, history, logger):
    ...
    try:
        ...
        for line_name, volume in volumes.items():
            if volume <= 0:
                logger.error(f"FAIL: Non-positive volume in {line_name.value}: {volume}")
                all_reasonable = False
        ...
        for line_name, gas_state in gas_network.lines.items():
            if gas_state.p > 1e6:  # > 10 bar
                logger.warning(f"WARN: Very high pressure in {line_name.value}: {gas_state.p:.0f}Pa")
