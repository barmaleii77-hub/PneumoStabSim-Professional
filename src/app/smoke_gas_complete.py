...
def run_complete_gas_smoke_test():
    ...
    try:
        ...
        logger.info(f"  - Receiver: p={gas_network.tank.p}Pa, T={gas_network.tank.T}K, V={gas_network.tank.V:.6f}m?")
        ...
        for step in range(steps):
            ...
            if step % int(0.5 / dt) == 0:
                logger.info(f"t={current_time:.2f}s - Tank: {gas_network.tank.p:.0f}Pa, Lines: [{', '.join(f'{gas.p:.0f}' for gas in gas_network.lines.values())}]Pa")
            
            advance_gas(dt, system, gas_network, ThermoMode.ISOTHERMAL, 
                       log=logger if step % int(1.0 / dt) == 0 else None)  # Detailed log every 1s
