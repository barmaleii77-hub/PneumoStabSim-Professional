...
from ..pneumo.enums import (
    Wheel, Line, Port, CheckValveKind, ReliefValveKind, ReceiverVolumeMode
)
...
def create_default_check_valve(kind: CheckValveKind) -> CheckValve:
    ...
    return CheckValve(
        kind=kind,
        delta_open_min=1000.0,  # 1000 Pa (10 mbar) minimum opening pressure
        d_eq=0.006,             # 6mm equivalent diameter
...
def create_default_receiver() -> ReceiverState:
    ...
    spec = ReceiverSpec(
        V_min=0.01,   # 10L minimum
        V_max=0.05    # 50L maximum
    )
    
    return ReceiverState(
        spec=spec,
        V=0.03,       # 30L initial volume
