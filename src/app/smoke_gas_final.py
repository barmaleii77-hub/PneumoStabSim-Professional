...
def run_full_smoke_test():
    ...
    try:
        ...
        logger.info(
            f"Initial tank: p={gas_network.tank.p:.0f}Pa, V={gas_network.tank.V:.6f}m3"
        )
        ...
        for step in range(steps):
            ...
            if step % 100 == 0:  # Every 0.5s
                ...
                logger.info(
                    f"t={current_time:.2f}s: Tank={gas_network.tank.p:.0f}Pa, Lines=[{min(pressures):.0f}-{max(pressures):.0f}]Pa"
                )
        ...
        print(
            f"p_line range: {min(final_pressures):.0f} - {max(final_pressures):.0f}Pa"
        )
