...
from .types import SourceKind, RoadConfig, validate_wheel_excitation, WHEEL_POSITIONS
...
class RoadInput:
    ...
    def get_profile_preview(self, duration: float = 10.0, 
                          dt: float = 0.01) -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
        ...
        # Get excitations
        preview_profiles = {wheel: np.zeros_like(t_preview) for wheel in ['LF', 'RF', 'LR', 'RR']}
    ...
    def _apply_correlation(self, t: np.ndarray, base_profile: np.ndarray, 
        ...
        if correlation and correlation.rho_LR < 1.0:
            ...
            noise = np.random.randn(len(base_profile)) * np.std(base_profile) * np.sqrt(1 - correlation.rho_LR**2)
    ...
    def _map_lr_to_wheels(self, t: np.ndarray, profiles_dict: Dict[str, np.ndarray]) -> None:
    ...
    def get_info(self) -> Dict[str, Any]:
        ...
        if self.time_base is not None:
            info.update({
                ...
                'sample_rate': len(self.time_base) / (self.time_base[-1] - self.time_base[0])
            })
...
__all__ = [
    'RoadInput', 'create_road_input_from_preset'
]