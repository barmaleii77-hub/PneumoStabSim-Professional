...
def generate_sine_profile(duration: float, velocity: float, amplitude: float, 
                         frequency: float, phase: float = 0.0, 
                         resample_hz: float = 1000.0) -> Tuple[np.ndarray, np.ndarray]:
...
def generate_sweep_profile(duration: float, velocity: float, amplitude: float,
                          f_start: float, f_end: float, sweep_type: str = 'logarithmic',
                          phase: float = 0.0, resample_hz: float = 1000.0) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if sweep_type == 'linear':
        ...
        profile = amplitude * signal.chirp(t, f_start, duration, f_end, 
                                         method='linear', phi=phase)
    elif sweep_type == 'logarithmic':
        # Logarithmic frequency sweep
        profile = amplitude * signal.chirp(t, f_start, duration, f_end,
                                         method='logarithmic', phi=phase)
    else:
...
def generate_step_profile(duration: float, velocity: float, step_height: float,
                         step_time: float, rise_time: float = 0.01,
                         resample_hz: float = 1000.0) -> Tuple[np.ndarray, np.ndarray]:
...
def generate_pothole_profile(duration: float, velocity: float, pothole_depth: float,
                           pothole_length: float, pothole_center_time: float,
                           resample_hz: float = 1000.0) -> Tuple[np.ndarray, np.ndarray]:
...
def generate_speed_bump_profile(duration: float, velocity: float, bump_height: float,
                              bump_length: float, bump_center_time: float,
...
def generate_iso8608_profile(duration: float, velocity: float, iso_class: Iso8608Class,
    ...
    noise_complex = (np.random.randn(len(freqs_pos)) + 
                    1j * np.random.randn(len(freqs_pos)))
    
    # Scale by PSD to get colored noise
    # Factor of 2 accounts for one-sided spectrum
    amplitude_spectrum = np.sqrt(2 * psd * (freqs_pos[1] - freqs_pos[0])) * noise_complex
    ...
    if correlation.method == 'coherence':
        ...
        noise_right_complex = (np.random.randn(len(freqs_pos)) + 
                             1j * np.random.randn(len(freqs_pos)))
        amplitude_right = np.sqrt(2 * psd * (freqs_pos[1] - freqs_pos[0])) * noise_right_complex
        ...
        if n_samples % 2 == 0:
            n_neg = len(freqs_pos) - 1
            if n_neg > 0:
                full_spectrum_right[-n_neg:] = np.conj(amplitude_right[1:n_neg+1][::-1])
        else:
            n_neg = len(freqs_pos) - 1
            if n_neg > 0:
                full_spectrum_right[-n_neg:] = np.conj(amplitude_right[1:n_neg+1][::-1])
...
def validate_iso8608_profile(profile: np.ndarray, velocity: float, iso_class: Iso8608Class,
                           resample_hz: float = 1000.0, 
                           tolerance: float = 0.5) -> Tuple[bool, Dict[str, Any]]:
...
# Export functions
__all__ = [
    'generate_sine_profile', 'generate_sweep_profile', 'generate_step_profile',
    'generate_pothole_profile', 'generate_speed_bump_profile', 'generate_iso8608_profile',
