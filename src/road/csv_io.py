    ...
    if len(preview_lines) >= 1:
        ...
        if header:
            ...
            if 'time' in clean_header:
                ...
                elif len(clean_header) == 5 and all(wheel in clean_header 
...
def load_csv_profile(filepath: str, format_type: str = 'auto', 
                    wheelbase: float = 3.2, velocity: float = 25.0,
                    correlation: Optional[CorrelationSpec] = None,
                    resample_hz: float = 1000.0) -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
    ...
    try:
        with open(filepath, 'r', encoding=encoding, newline='') as f:
            ...
            # Read data
            data_rows = []
            for row_num, row in enumerate(reader, start=2):  # Start from line 2 (after header)
                try:
                    ...
                    for i, cell in enumerate(row):
                        ...
                        try:
                            ...
                            warnings.warn(f"Invalid numeric value '{cell}' at row {row_num}, column {i+1}")
                            continue
    ...
    if format_type == 'time_z':
        ...
        if data.shape[1] < 2:
            raise ValueError(f"time_z format requires at least 2 columns, got {data.shape[1]}")
        ...
        if data.shape[1] < 5:
            raise ValueError(f"time_wheels format requires 5 columns, got {data.shape[1]}")
        ...
        for wheel in ['lf', 'rf', 'lr', 'rr']:
            try:
                ...
                raise ValueError(f"Wheel column '{wheel}' not found in header: {header}")
        ...
        if data.shape[1] < 4:
            raise ValueError(f"wheels_only format requires 4 columns, got {data.shape[1]}")
    ...
    if not np.all(np.diff(time_uniform) > 0):
        warnings.warn("Time array is not strictly monotonic - interpolation may be unreliable")
    ...
    for wheel, profile in wheel_profiles.items():
        if not np.all(np.isfinite(profile)):
            ...
            if np.sum(mask) >= 2:
                interp_func = interp1d(time_uniform[mask], profile[mask], 
...
def _expand_single_profile_to_wheels(time_orig: np.ndarray, z_profile: np.ndarray,
                                   wheelbase: float, velocity: float,
                                   correlation: CorrelationSpec,
                                   resample_hz: float) -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
    ...
    time_uniform = np.linspace(time_orig[0], time_orig[0] + duration, 
                              int(duration * resample_hz))
    
    # Interpolate original profile to uniform grid
    interp_func = interp1d(time_orig, z_profile, kind='linear', 
    ...
    if correlation.rho_LR < 1.0:
        ...
        noise = np.random.randn(len(z_uniform)) * np.std(z_uniform) * np.sqrt(1 - correlation.rho_LR**2)
    ...
    interp_rear_left = interp1d(time_uniform, z_uniform, kind='linear',
                               bounds_error=False, fill_value=0.0)
    interp_rear_right = interp1d(time_uniform, z_right, kind='linear',
                                bounds_error=False, fill_value=0.0)
...
def _resample_wheel_profiles(time_orig: np.ndarray, wheel_data: Dict[str, np.ndarray],
                           resample_hz: float) -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
    """Resample wheel profiles to uniform time grid"""
    
    duration = time_orig[-1] - time_orig[0]
    time_uniform = np.linspace(time_orig[0], time_orig[-1], 
                              int(duration * resample_hz))
    
    wheel_profiles = {}
    
    for wheel, profile in wheel_data.items():
        interp_func = interp1d(time_orig, profile, kind='linear',
...
def save_csv_profile(filepath: str, time: np.ndarray, wheel_profiles: Dict[str, np.ndarray],
                    format_type: str = 'time_wheels', encoding: str = 'utf-8') -> None:
...
# Export functions
__all__ = [
    'detect_csv_format', 'load_csv_profile', 'save_csv_profile'
]