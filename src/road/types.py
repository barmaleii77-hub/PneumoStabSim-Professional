...
class CorrelationSpec:
    ...
    def __post_init__(self):
        ...
        if self.method not in ['coherence', 'mixing']:
            raise ValueError(f"method must be 'coherence' or 'mixing', got {self.method}")
...
def validate_wheel_excitation(excitation: Dict[str, float]) -> bool:
    ...
    if set(excitation.keys()) != required_keys:
        return False
    return all(isinstance(v, (int, float)) and np.isfinite(v) for v in excitation.values())


# Export main types
__all__ = [
    'SourceKind', 'Iso8608Class', 'CorrelationSpec', 'Preset', 'RoadConfig',
    'ISO8608_PARAMETERS', 'WHEEL_POSITIONS', 'WheelPosition',
