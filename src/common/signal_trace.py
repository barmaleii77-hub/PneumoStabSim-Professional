...
class SignalTraceConfig:
    ...
    def from_dict(cls, data: Dict[str, Any] | None) -> "SignalTraceConfig":
        ...
        exclude = tuple(str(item) for item in raw_exclude if isinstance(item, str) and item)
        ...
        if isinstance(history_value, bool):
            ...
            history_candidate = history_value if isinstance(history_value, (int, float, str)) else None
...
class SignalTraceService(QObject):
    ...
    # ------------------------------------------------------------------
    # Public API used from QML/Python
    # ------------------------------------------------------------------
