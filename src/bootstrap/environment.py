    ...
    try:
        ...
        if os.path.isdir(local_qml):
            ...
            if local_qml not in existing:
                os.environ["QML2_IMPORT_PATH"] = (existing + os.pathsep + local_qml) if existing else local_qml
            existing2 = os.environ.get("QML_IMPORT_PATH", "")
            if local_qml not in existing2:
                os.environ["QML_IMPORT_PATH"] = (existing2 + os.pathsep + local_qml) if existing2 else local_qml
