...
if not TYPE_CHECKING and (sentinel := sys.modules.get(_SENTINEL_MODULE)) is not None:
    HeadlessApplication = sentinel.HeadlessApplication  # type: ignore[attr-defined]
    HeadlessQtNamespace = sentinel.HeadlessQtNamespace  # type: ignore[attr-defined]
    HeadlessTimer = sentinel.HeadlessTimer  # type: ignore[attr-defined]
    headless_install_message_handler = sentinel.headless_install_message_handler  # type: ignore[attr-defined]
else:
    ...
    class HeadlessQtNamespace:
        """Namespace exposing Qt attributes used by :class:`ApplicationRunner`."""
    ...
    sentinel_module.HeadlessQtNamespace = HeadlessQtNamespace
    sentinel_module.HeadlessTimer = HeadlessTimer
    sentinel_module.headless_install_message_handler = headless_install_message_handler
    sys.modules[_SENTINEL_MODULE] = sentinel_module
