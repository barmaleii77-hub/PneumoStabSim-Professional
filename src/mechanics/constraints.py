...
Implements:
- Track <=? arm_length + pivot_offset invariant (see validate_track_invariant)
...
class ConstraintValidator:
    ...
    def validate_max_vertical_travel(
        self, arm_length: float, max_free_end_y: float
    ) -> bool:
        ...
        # Conservative limit: |y| <= arm_length (total range =2*arm_length)
        return abs(max_free_end_y) <= arm_length

    def validate_rod_attach_fraction(self, fraction: float) -> bool:
        """Validate rod attachment point fraction in [0.1,0.9]"""
        return (
            self.bounds.min_rod_attach_fraction
            <= fraction
    ...
    def validate_residual_volume(
        ...
        Args:
            residual_volume: Dead zone volume (m^3)
    ...
    def validate_geometry_params(
        ...
        if not self.validate_track_invariant(wheelbase, arm_length, pivot_offset):
            errors.append(
                f"Track invariant violated: {wheelbase:.3f} vs2*({arm_length:.3f}+{pivot_offset:.3f})"
            )
        ...
        if not (
            self.bounds.min_pivot_offset <= pivot_offset <= self.bounds.max_pivot_offset
        ):
            errors.append(
...
def calculate_min_residual_volume(
    diameter: float, stroke: float, fraction: float = 0.005
) -> float:
    """Calculate minimum residual volume (default0.5%)

    Args:
    diameter: Inner diameter (m)
    stroke: Full stroke (m)
    fraction: Residual fraction (default0.005 =0.5%)

    Returns:
    Minimum residual volume (m^3)
    """
