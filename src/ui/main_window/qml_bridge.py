...
try:
 ...
 def Q_ARG(*_args, **_kwargs):
 return None

 QMetaObject = _DummyQMetaObject
 Qt = _DummyQt
...
class QMLUpdateError(RuntimeError):
 """Исключение, выбрасываемое при критическом сбое обновления QML."""
...
class QMLBridge:
 ...
 # Schedule immediate flush if timer available
 try:
 if not window._qml_flush_timer.isActive():
 window._qml_flush_timer.start(0)
 except Exception:
 # Timer may be a test stub without isActive
 pass
 ...
 Стратегия: сначала попытаться записать в property, затем вызывать
 индивидуальные методы как fallback.
 """
 if not window._qml_update_queue:
 return

 if not getattr(window, "_qml_root_object", None):
 # Retry later if root not ready
 try:
 ...
 result = QMLBridge._push_batched_updates(window, pending, detailed=True)
 ...
 # Notify user and record failure
 QMLBridge._notify_qml_failure(window, "QML не принял пакет обновлений", failure_reason)
 ...
 *,
 ...
 payload.setdefault("animation", {})["isRunning"] = bool(getattr(window, "is_simulation_running", False))

 sanitized = QMLBridge._prepare_for_qml(payload)
 window._qml_root_object.setProperty("pendingPythonUpdates", sanitized)
 return True
 except Exception as exc:
 QMLBridge.logger.error("Failed to push simulation state to QML: %s", exc, exc_info=True)
 try:
 QMLBridge._log_qml_update_failure(window, context="set_simulation_state", payload=payload or {}, error=exc)
 except Exception:
 QMLBridge.logger.debug("Failed to record simulation state failure into event logger", exc_info=True)

 QMLBridge._notify_qml_failure(window, "QML не принял состояние симуляции", str(exc))
 return False
 ...
 # ------------------------------------------------------------------
 @staticmethod
 def invoke_qml_function(window: "MainWindow", method_name: str, payload: Optional[Dict[str, Any]] = None) -> bool:
 ...
 if payload is None:
 success = QMetaObject.invokeMethod(window._qml_root_object, method_name, Qt.ConnectionType.DirectConnection)
 else:
