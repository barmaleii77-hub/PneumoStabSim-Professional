...
class SignalsRouter:
    ...
    def _connect_panel_signals(window: MainWindow) -> None:
        ...
        if window.modes_panel:
            ...
            window.modes_panel.animation_changed.connect(
        ...
        if window.graphics_panel:
            window.graphics_panel.lighting_changed.connect(
                window._on_lighting_changed
            )
            window.graphics_panel.material_changed.connect(
                window._on_material_changed
            )
            window.graphics_panel.environment_changed.connect(
                window._on_environment_changed
            )
            window.graphics_panel.quality_changed.connect(
                window._on_quality_changed
            )
            window.graphics_panel.camera_changed.connect(
                window._on_camera_changed
            )
            window.graphics_panel.effects_changed.connect(
                window._on_effects_changed
            )
            window.graphics_panel.preset_applied.connect(
                window._on_preset_applied
            )
    ...
    def _connect_simulation_signals(window: MainWindow) -> None:
        ...
        try:
            ...
            bus.state_ready.connect(
                window._on_state_update, Qt.QueuedConnection
            )
            bus.physics_error.connect(
                window._on_physics_error, Qt.QueuedConnection
            )
