...
class PressureScaleWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        while p <= self.p_max:
            ...
            painter.drawLine(scale_x + scale_width, int(y), scale_x + scale_width + 5, int(y))
        ...
        self._draw_marker(painter, self.p_atm, "Patm", QColor(150, 150, 150), scale_x, scale_width)
        self._draw_marker(painter, self.p_min_relief, "Min", QColor(100, 200, 100), scale_x, scale_width)
        self._draw_marker(painter, self.p_stiff, "Stiff", QColor(255, 200, 0), scale_x, scale_width)
        self._draw_marker(painter, self.p_safety, "Safe", QColor(255, 100, 100), scale_x, scale_width)
        
        # Draw tank pressure indicator
        y_tank = self.map_pressure_to_height(self.p_tank)
        painter.setPen(QPen(QColor(255, 255, 0), 3))
        painter.drawLine(scale_x - 5, int(y_tank), scale_x + scale_width + 5, int(y_tank))
    ...
    def _draw_marker(self, painter: QPainter, pressure: float, label: str, 
                     color: QColor, x: int, width: int):
...
class TankOverlayHUD:
    ...
    def render(self, painter: QPainter, screen_width: int, screen_height: int):
        ...
        for i, p_line in enumerate(self.p_lines):
            ...
            painter.drawEllipse(QPointF(sphere_x, sphere_y), sphere_radius, sphere_radius)
