...
class MainWindow(QMainWindow):
    ...
    def _setup_qml_3d_view(self):
        ...
        try:
            ...
            print(
                f"    [DEBUG] QML root object class: {self._qml_root_object.__class__.__name__ if self._qml_root_object else 'None'}"
            )
            ...
            try:
                ...
                fallback.setStyleSheet(
                    "background: #1a1a2e; color: #ff6b6b; font-size: 14px; padding: 20px;"
                )
    ...
    def _wire_panel_signals(self):
        ...
        if self.road_panel:
            ...
            self.road_panel.apply_to_wheels.connect(
                lambda pname, wheels: self.logger.info(
                    f"Apply {pname} to wheels {wheels}"
                )
            )
    ...
    def _setup_menus(self):
        ...
        load_csv_act.triggered.connect(
            lambda: self.road_panel and self.road_panel._browse_csv_file()
        )
        clear_profiles_act.triggered.connect(
            lambda: self.road_panel and self.road_panel._clear_all_profiles()
        )
    ...
    def _update_3d_scene_from_snapshot(self, snapshot: StateSnapshot):
        ...
        try:
            ...
            if current_params != self._last_animation_params:
                print(
                    f"📊 Animation params: Amp={amplitude:.1f}° Freq={frequency:.2f}Hz Phase={phase_global:.0f}°"
                )
            ...
            corners_data = {
                "fl": {
                    ...
                    * np.sin(omega * t + np.deg2rad(phase_global + phase_fl)),
                    "cylinderState": None,
                },
                "fr": {
                    "leverAngle": amplitude
                    * np.sin(omega * t + np.deg2rad(phase_global + phase_fr)),
                    "cylinderState": None,
                },
                "rl": {
                    "leverAngle": amplitude
            ...
            if piston_positions:
                ...
                if not success_pistons:
                    self.logger.warning(
                        "Failed to invoke updatePistonPositions() in QML"
                    )
    ...
    def _on_geometry_changed(self, geometry_params: dict):
        ...
        """
        print("═══════════════════════════════════════════════")
        print("📥 MainWindow: Received geometry_changed signal")
        ...
        if self._qml_root_object:
            try:
                ...
                print("═══════════════════════════════════════════════")
                print("❌ QML geometry update FAILED!")
                print(f"   Error: {e}")
                print("═══════════════════════════════════════════════")
            ...
            print("═══════════════════════════════════════════════")
            print("❌ MainWindow: QML root object is None!")
            print("   Cannot update geometry")
            print("═══════════════════════════════════════════════")
    ...
    def _set_geometry_properties_fallback(self, geometry_params: dict):
        ...
        """
        print("🔧 Fallback: Setting individual QML properties...")

        prop_count = 0

        if "frameLength" in geometry_params:
            self._qml_root_object.setProperty(
                "userFrameLength", geometry_params["frameLength"]
            )
        ...
        if "frameHeight" in geometry_params:
            self._qml_root_object.setProperty(
                "userFrameHeight", geometry_params["frameHeight"]
            )
        ...
        if "frameBeamSize" in geometry_params:
            self._qml_root_object.setProperty(
                "userBeamSize", geometry_params["frameBeamSize"]
            )
        ...
        if "leverLength" in geometry_params:
            self._qml_root_object.setProperty(
                "userLeverLength", geometry_params["leverLength"]
            )
        ...
        if "cylinderBodyLength" in geometry_params:
            self._qml_root_object.setProperty(
                "userCylinderLength", geometry_params["cylinderBodyLength"]
            )
            print(
                f"   ✅ Set userCylinderLength = {geometry_params['cylinderBodyLength']}"
            )
            prop_count += 1
        ...
        if "trackWidth" in geometry_params:
            self._qml_root_object.setProperty(
                "userTrackWidth", geometry_params["trackWidth"]
            )
        ...
        if "frameToPivot" in geometry_params:
            self._qml_root_object.setProperty(
                "userFrameToPivot", geometry_params["frameToPivot"]
            )
        ...
        if "rodPosition" in geometry_params:
            self._qml_root_object.setProperty(
                "userRodPosition", geometry_params["rodPosition"]
            )
        ...
        if "boreHead" in geometry_params:
            self._qml_root_object.setProperty(
                "userBoreHead", geometry_params["boreHead"]
            )
        ...
        if "rodDiameter" in geometry_params:
            self._qml_root_object.setProperty(
                "userRodDiameter", geometry_params["rodDiameter"]
            )
        ...
        if "pistonThickness" in geometry_params:
            self._qml_root_object.setProperty(
                "userPistonThickness", geometry_params["pistonThickness"]
            )
            print(
                f"   ✅ Set userPistonThickness = {geometry_params['pistonThickness']}"
            )
            prop_count += 1

        self.status_bar.showMessage(
            f"Geometry properties updated ({prop_count} properties)"
        )
        ...
        if self._qml_root_object:
            try:
                ...
                if "amplitude" in animation_params:
                    ...
                    amplitude_deg = (
                        animation_params["amplitude"] * 1000 / 10
                ...
                if "frequency" in animation_params:
                    self._qml_root_object.setProperty(
                        "userFrequency", animation_params["frequency"]
                    )
                    print(
                        f"   ✅ Set userFrequency = {animation_params['frequency']} Hz"
                    )
    ...
    def _save_preset(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Save UI Preset", last_dir, "JSON Files (*.json)"
        )
        if not file_path:
            return
    ...
    def _load_preset(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Load UI Preset", last_dir, "JSON Files (*.json)"
        )
        if not file_path:
            return
        settings.setValue(self.SETTINGS_LAST_PRESET, str(Path(file_path).parent))
    ...
    def _export_timeseries(self):
        ...
        try:
            ...
            QMessageBox.warning(
                self, "Export", "Chart widget does not support export yet"
            )
            return
        ...
        try:
            ...
            self.status_bar.showMessage(
                f"Exported {len(time)} points to {file_path.name}"
            )
    ...
    def _export_snapshots(self):
        ...
        try:
            ...
            self.status_bar.showMessage(
                f"Exported {len(snapshots)} snapshots to {file_path.name}"
            )
            QMessageBox.information(
                self,
                "Export Successful",
