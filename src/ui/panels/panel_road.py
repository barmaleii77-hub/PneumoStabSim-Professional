...
class RoadPanel(QWidget):
    ...
    def _create_assignment_group(self) -> QGroupBox:
        ...
        # Make them smaller
        for btn in [self.apply_lf_button, self.apply_rf_button, 
                   self.apply_lr_button, self.apply_rr_button]:
        ...
        for btn in [self.apply_all_button, self.apply_front_button, self.apply_rear_button,
                   self.apply_lf_button, self.apply_rf_button, self.apply_lr_button, self.apply_rr_button]:
    ...
    def _connect_signals(self):
        ...
        self.apply_lf_button.clicked.connect(
            lambda: self._apply_to_wheels(["LF"]))
        self.apply_rf_button.clicked.connect(
            lambda: self._apply_to_wheels(["RF"]))
        self.apply_lr_button.clicked.connect(
            lambda: self._apply_to_wheels(["LR"]))
        self.apply_rr_button.clicked.connect(
            lambda: self._apply_to_wheels(["RR"]))
    ...
    def _browse_csv_file(self):
        ...
        if file_path:
            ...
            try:
                ...
                self.info_text.setPlainText(f"Selected file: {filename}\nSize: {file_size} bytes")
    ...
    def _apply_current_preset(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "Error Applying Preset",
    ...
    def _apply_to_wheels(self, wheels: list):
        ...
        if self.current_csv_path:
            ...
            QMessageBox.warning(self, "No Profile", "No profile is currently loaded or selected.")
            return
        ...
        self.info_text.setPlainText(f"{current_info}\n\nApplied to wheels: {wheels_str}")
