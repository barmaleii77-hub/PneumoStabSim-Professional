...
class GeometryStateManager:
    ...
    def _get_hydraulic_warnings(self) -> List[str]:
        ...
        if (rod_diameter_m >= cyl_diam_m * WARNING_ROD_TO_CYLINDER_RATIO and
            rod_diameter_m < cyl_diam_m * MAX_ROD_TO_CYLINDER_RATIO):
            warnings.append(
    ...
    def check_dependencies(self, param_name: str, new_value: Any, 
                          old_value: Any) -> Optional[Dict[str, Any]]:
    ...
    def _check_hydraulic_dependency(self, param_name: str, 
                                   new_value: float) -> Optional[Dict[str, Any]]:
        ...
        if rod_diameter_m >= cyl_diam_m * MAX_ROD_TO_CYLINDER_RATIO:
            return {
                ...
                'options': [
                    ('Уменьшить диаметр штока', 'rod_diameter_m', 
                     cyl_diam_m * WARNING_ROD_TO_CYLINDER_RATIO),
                    ('Увеличить диаметр цилиндра', 'cyl_diam_m', 
    ...
    def _check_geometric_dependency(self, param_name: str, 
                                   new_value: float) -> Optional[Dict[str, Any]]:
        ...
        if frame_to_pivot + lever_length > max_lever_reach:
            return {
                ...
                'options': [
                    ('Уменьшить длину рычага', 'lever_length', 
                     max_lever_reach - frame_to_pivot),
                    ('Уменьшить расстояние до оси', 'frame_to_pivot',
                     max_lever_reach - lever_length),
                    ('Увеличить колёсную базу', 'wheelbase',
