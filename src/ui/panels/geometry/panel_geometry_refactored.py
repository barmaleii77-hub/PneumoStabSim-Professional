...
class GeometryPanel(QWidget):
    ...
    def _connect_signals(self):
        ...
        self.frame_tab.parameter_live_changed.connect(self._on_tab_parameter_live_changed)
        
        # Suspension tab
        self.suspension_tab.parameter_changed.connect(self._on_tab_parameter_changed)
        self.suspension_tab.parameter_live_changed.connect(self._on_tab_parameter_live_changed)
        
        # Cylinder tab
        self.cylinder_tab.parameter_changed.connect(self._on_tab_parameter_changed)
        self.cylinder_tab.parameter_live_changed.connect(self._on_tab_parameter_live_changed)
    ...
    def _update_tab_widget(self, param_name: str, value: float):
        ...
        if param_name in ['wheelbase', 'track']:
            ...
            self.suspension_tab.update_from_state()
        elif param_name in ['cylinder_length', 'cyl_diam_m', 'stroke_m', 
                          'dead_gap_m', 'rod_diameter_m', 'piston_rod_length_m', 
                          'piston_thickness_m']:
            self.cylinder_tab.update_from_state()
    ...
    def _get_3d_update_params() -> set:
        ...
        """
        return {
            'wheelbase', 'track', 'lever_length', 'cylinder_length',
            'frame_to_pivot', 'rod_position', 'cyl_diam_m', 'stroke_m',
            'dead_gap_m', 'rod_diameter_m', 'piston_rod_length_m',
            'piston_thickness_m'
        }
