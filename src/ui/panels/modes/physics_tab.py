...
class PhysicsTab(QWidget):
    ...
    def _create_components_group(self) -> QGroupBox:
        ...
        info_label.setStyleSheet("color: #666666; background-color: #f5f5f5; padding: 8px; border-radius: 4px;")
        info_font = QFont()
    ...
    def _on_option_changed(self, option_name: str, checked: bool):
        ...
        self.physics_options_changed.emit(
            self.state_manager.get_physics_options()
        )
    ...
    def _apply_current_state(self):
        ...
        self.include_springs_check.setChecked(
            options.get('include_springs', True)
        )
        self.include_dampers_check.setChecked(
            options.get('include_dampers', True)
        )
