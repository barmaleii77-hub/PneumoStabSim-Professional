...
class StandardSlider(QWidget):
    ...
    def _setup_ui(self, title):
        ...
        controls_layout = QHBoxLayout(); controls_layout.setSpacing(4)
...
class ModesPanel(QWidget):
    ...
    def _create_control_group(self) -> QGroupBox:
        ...
        buttons_layout = QHBoxLayout(); buttons_layout.setSpacing(8)
        self.start_button = QPushButton("▶ Старт")
        self.stop_button = QPushButton("⏹ Стоп")
        self.pause_button = QPushButton("⏸ Пауза")
        self.reset_sim_button = QPushButton("🔄 Сброс")
        for b in (self.start_button, self.stop_button, self.pause_button, self.reset_sim_button):
    ...
    def _create_mode_group(self) -> QGroupBox:
        ...
        sim_type_widget = QWidget(); sim_type_layout = QVBoxLayout(sim_type_widget); sim_type_layout.setSpacing(4)
        sim_type_label = QLabel("Режим физики"); sim_type_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        font = QFont(); font.setPointSize(9); font.setBold(True); sim_type_label.setFont(font)
        ...
        thermo_widget = QWidget(); thermo_layout = QVBoxLayout(thermo_widget); thermo_layout.setSpacing(4)
        thermo_label = QLabel("Термо-режим"); thermo_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        font = QFont(); font.setPointSize(9); font.setBold(True); thermo_label.setFont(font)
    ...
    def _create_road_group(self) -> QGroupBox:
        ...
        self.amplitude_slider = StandardSlider(minimum=0.0, maximum=0.2, value=float(self.parameters.get('amplitude', 0) or 0), step=0.001, decimals=3, units="м", title="Глобальная амплитуда")
        layout.addWidget(self.amplitude_slider)
        self.frequency_slider = StandardSlider(minimum=0.1, maximum=10.0, value=float(self.parameters.get('frequency', 0) or 0), step=0.1, decimals=1, units="Гц", title="Глобальная частота")
        layout.addWidget(self.frequency_slider)
        self.phase_slider = StandardSlider(minimum=0.0, maximum=360.0, value=float(self.parameters.get('phase', 0) or 0), step=15.0, decimals=0, units="°", title="Глобальная фаза")
        layout.addWidget(self.phase_slider)

        per_wheel_label = QLabel("Фазовые сдвиги по колёсам"); font = QFont(); font.setPointSize(9); font.setBold(True); per_wheel_label.setFont(font)
        layout.addWidget(per_wheel_label)

        wheel_layout = QHBoxLayout(); wheel_layout.setSpacing(8)
        lf_widget = QWidget(); lf_layout = QVBoxLayout(lf_widget); lf_layout.setContentsMargins(2, 2, 2, 2); lf_layout.setSpacing(2)
        lf_label = QLabel("ЛП"); lf_label.setAlignment(Qt.AlignmentFlag.AlignCenter); lf_layout.addWidget(lf_label)
        self.lf_phase_slider = StandardSlider(minimum=0.0, maximum=360.0, value=float(self.parameters.get('lf_phase', 0) or 0), step=15.0, decimals=0, units="°", title="")
        lf_layout.addWidget(self.lf_phase_slider); wheel_layout.addWidget(lf_widget)

        rf_widget = QWidget(); rf_layout = QVBoxLayout(rf_widget); rf_layout.setContentsMargins(2, 2, 2, 2); rf_layout.setSpacing(2)
        rf_label = QLabel("ПП"); rf_label.setAlignment(Qt.AlignmentFlag.AlignCenter); rf_layout.addWidget(rf_label)
        self.rf_phase_slider = StandardSlider(minimum=0.0, maximum=360.0, value=float(self.parameters.get('rf_phase', 0) or 0), step=15.0, decimals=0, units="°", title="")
        rf_layout.addWidget(self.rf_phase_slider); wheel_layout.addWidget(rf_widget)

        lr_widget = QWidget(); lr_layout = QVBoxLayout(lr_widget); lr_layout.setContentsMargins(2, 2, 2, 2); lr_layout.setSpacing(2)
        lr_label = QLabel("ЛЗ"); lr_label.setAlignment(Qt.AlignmentFlag.AlignCenter); lr_layout.addWidget(lr_label)
        self.lr_phase_slider = StandardSlider(minimum=0.0, maximum=360.0, value=float(self.parameters.get('lr_phase', 0) or 0), step=15.0, decimals=0, units="°", title="")
        lr_layout.addWidget(self.lr_phase_slider); wheel_layout.addWidget(lr_widget)

        rr_widget = QWidget(); rr_layout = QVBoxLayout(rr_widget); rr_layout.setContentsMargins(2, 2, 2, 2); rr_layout.setSpacing(2)
        rr_label = QLabel("ПЗ"); rr_label.setAlignment(Qt.AlignmentFlag.AlignCenter); rr_layout.addWidget(rr_label)
        self.rr_phase_slider = StandardSlider(minimum=0.0, maximum=360.0, value=float(self.parameters.get('rr_phase', 0) or 0), step=15.0, decimals=0, units="°", title="")
        rr_layout.addWidget(self.rr_phase_slider); wheel_layout.addWidget(rr_widget)
    ...
    def _apply_state_to_widgets(self) -> None:
        ...
        if preset and preset != 'Пользовательский':
            ...
            idx = self.mode_preset_combo.findText(preset) if isinstance(preset, str) else -1
            if idx >= 0:
        ...
        # Physics options
        self.include_springs_check.setChecked(bool(self.physics_options.get('include_springs', False)))
        self.include_dampers_check.setChecked(bool(self.physics_options.get('include_dampers', False)))
        self.include_pneumatics_check.setChecked(bool(self.physics_options.get('include_pneumatics', False)))
    ...
    def _connect_signals(self):
        ...
        self.reset_sim_button.clicked.connect(lambda: self.simulation_control.emit("reset"))
        ...
        self.include_springs_check.toggled.connect(lambda checked: self._on_physics_option_changed('include_springs', checked))
        self.include_dampers_check.toggled.connect(lambda checked: self._on_physics_option_changed('include_dampers', checked))
        self.include_pneumatics_check.toggled.connect(lambda checked: self._on_physics_option_changed('include_pneumatics', checked))

        # Road excitation sliders
        self.amplitude_slider.valueEdited.connect(lambda v: self._on_parameter_changed('amplitude', v))
        self.frequency_slider.valueEdited.connect(lambda v: self._on_parameter_changed('frequency', v))
        self.phase_slider.valueEdited.connect(lambda v: self._on_parameter_changed('phase', v))
        self.lf_phase_slider.valueEdited.connect(lambda v: self._on_parameter_changed('lf_phase', v))
        self.rf_phase_slider.valueEdited.connect(lambda v: self._on_parameter_changed('rf_phase', v))
        self.lr_phase_slider.valueEdited.connect(lambda v: self._on_parameter_changed('lr_phase', v))
        self.rr_phase_slider.valueEdited.connect(lambda v: self._on_parameter_changed('rr_phase', v))

    @Slot(int)
    def _on_mode_preset_changed(self, index: int):
        presets = {
            0: {
                'sim_type': 'KINEMATICS', 'thermo_mode': 'ISOTHERMAL',
                'include_springs': True, 'include_dampers': True, 'include_pneumatics': True
            },
            1: {
                'sim_type': 'KINEMATICS', 'thermo_mode': 'ISOTHERMAL',
                'include_springs': False, 'include_dampers': False, 'include_pneumatics': False
            },
            2: {
                'sim_type': 'DYNAMICS', 'thermo_mode': 'ADIABATIC',
                'include_springs': True, 'include_dampers': True, 'include_pneumatics': True
            },
            3: {
                'sim_type': 'DYNAMICS', 'thermo_mode': 'ISOTHERMAL',
                'include_springs': False, 'include_dampers': False, 'include_pneumatics': True
            },
            4: { 'custom': True }
        }
        ...
        if 'custom' not in preset:
            ...
            self.include_pneumatics_check.setChecked(preset.get('include_pneumatics', False))
