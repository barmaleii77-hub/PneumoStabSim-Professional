...
class PneumoPanel(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.pressure_units_combo.currentIndexChanged.connect(
            self._on_pressure_units_changed
        )
    ...
    def _connect_signals(self) -> None:
        ...
        self._connect_knob(
            self.receiver_diameter_knob, lambda _v: self._on_receiver_geometry_changed()
        )
        self._connect_knob(
            self.receiver_length_knob, lambda _v: self._on_receiver_geometry_changed()
        )

        # Check valves
        self._connect_knob(
            self.cv_atmo_dp_knob,
            lambda v: self._on_parameter_changed("cv_atmo_dp", float(v)),
        )
        self._connect_knob(
            self.cv_tank_dp_knob,
            lambda v: self._on_parameter_changed("cv_tank_dp", float(v)),
        )
        self._connect_knob(
            self.cv_atmo_dia_knob,
            lambda v: self._on_parameter_changed("cv_atmo_dia", float(v)),
        )
        self._connect_knob(
            self.cv_tank_dia_knob,
            lambda v: self._on_parameter_changed("cv_tank_dia", float(v)),
        )

        # Relief valves
        self._connect_knob(
            self.relief_min_pressure_knob,
            lambda v: self._on_parameter_changed("relief_min_pressure", float(v)),
        )
        self._connect_knob(
            self.relief_stiff_pressure_knob,
            lambda v: self._on_parameter_changed("relief_stiff_pressure", float(v)),
        )
        self._connect_knob(
            self.relief_safety_pressure_knob,
            lambda v: self._on_parameter_changed("relief_safety_pressure", float(v)),
        )
        self._connect_knob(
            self.throttle_min_dia_knob,
            lambda v: self._on_parameter_changed("throttle_min_dia", float(v)),
        )
        self._connect_knob(
            self.throttle_stiff_dia_knob,
            lambda v: self._on_parameter_changed("throttle_stiff_dia", float(v)),
        )

        # Environment
        self._connect_knob(
            self.atmo_temp_knob,
            lambda v: self._on_parameter_changed("atmo_temp", float(v)),
        )
        ...
        self.master_isolation_check.toggled.connect(
            lambda checked: self._on_parameter_changed(
    ...
    def _create_receiver_group(self) -> QGroupBox:
        ...
        self.manual_volume_knob = Knob(
            minimum=0.001,
            maximum=0.100,
            value=float(self.parameters.get("receiver_volume", 0) or 0),
            step=0.001,
            decimals=3,
            units="м³",
        ...
        self.receiver_diameter_knob = Knob(
            minimum=0.050,
            maximum=0.500,
            value=float(self.parameters.get("receiver_diameter", 0) or 0),
            step=0.001,
            decimals=3,
            units="м",
            title="Диаметр ресивера",
        )
        geometric_layout.addWidget(self.receiver_diameter_knob)
        self.receiver_length_knob = Knob(
            minimum=0.100,
            maximum=2.000,
            value=float(self.parameters.get("receiver_length", 0) or 0),
            step=0.001,
            decimals=3,
            units="м",
            title="Длина ресивера",
        )
    ...
    def _create_check_valves_group(self) -> QGroupBox:
        ...
        self.cv_atmo_dp_knob = Knob(
            minimum=0.001,
            maximum=0.1,
            value=float(self.parameters.get("cv_atmo_dp", 0) or 0),
            step=0.001,
            decimals=3,
            units="бар",
            title="ΔP Атм→Линия",
        ...
        self.cv_tank_dp_knob = Knob(
            minimum=0.001,
            maximum=0.1,
            value=float(self.parameters.get("cv_tank_dp", 0) or 0),
            step=0.001,
            decimals=3,
            units="бар",
        ...
        self.cv_atmo_dia_knob = Knob(
            minimum=1.0,
            maximum=10.0,
            value=float(self.parameters.get("cv_atmo_dia", 0) or 0),
            step=0.1,
            decimals=1,
            units="мм",
            title="Диаметр (Атм)",
        ...
        self.cv_tank_dia_knob = Knob(
            minimum=1.0,
            maximum=10.0,
            value=float(self.parameters.get("cv_tank_dia", 0) or 0),
            step=0.1,
            decimals=1,
            units="мм",
    ...
    def _create_relief_valves_group(self) -> QGroupBox:
        ...
        self.relief_min_pressure_knob = Knob(
            minimum=1.0,
            maximum=10.0,
            value=float(self.parameters.get("relief_min_pressure", 0) or 0),
            step=0.1,
            decimals=1,
            units="бар",
            title="Мин. сброс",
        ...
        self.relief_stiff_pressure_knob = Knob(
            minimum=5.0,
            maximum=50.0,
            value=float(self.parameters.get("relief_stiff_pressure", 0) or 0),
            step=0.5,
            decimals=1,
            units="бар",
        ...
        self.relief_safety_pressure_knob = Knob(
            minimum=20.0,
            maximum=100.0,
            value=float(self.parameters.get("relief_safety_pressure", 0) or 0),
            step=1.0,
            decimals=0,
            units="бар",
        ...
        self.throttle_min_dia_knob = Knob(
            minimum=0.5,
            maximum=3.0,
            value=float(self.parameters.get("throttle_min_dia", 0) or 0),
            step=0.1,
            decimals=1,
            units="мм",
            title="Дроссель мин.",
        ...
        self.throttle_stiff_dia_knob = Knob(
            minimum=0.5,
            maximum=3.0,
            value=float(self.parameters.get("throttle_stiff_dia", 0) or 0),
            step=0.1,
            decimals=1,
            units="мм",
    ...
    def _create_environment_group(self) -> QGroupBox:
        ...
        self.atmo_temp_knob = Knob(
            minimum=-20.0,
            maximum=50.0,
            value=float(self.parameters.get("atmo_temp", 0) or 0),
            step=1.0,
            decimals=0,
            units="°C",
            title="Температура атм.",
        )
    ...
    def _on_receiver_geometry_changed(self):
        if self.parameters.get("volume_mode") == "GEOMETRIC":
            ...
            self.parameter_changed.emit(
                "receiver_volume", float(self.parameters["receiver_volume"])
            )
            self.receiver_volume_changed.emit(
                float(self.parameters["receiver_volume"]), "GEOMETRIC"
            )
            self.pneumatic_updated.emit(self.parameters.copy())
    ...
    def _on_volume_mode_changed(self, index: int):
        if index == 0:
            ...
            self.receiver_volume_changed.emit(
                float(self.parameters["receiver_volume"]), "MANUAL"
            )
        else:
    ...
    def _reset_to_defaults(self):
        ...
        self.volume_mode_combo.setCurrentIndex(
            0 if self.parameters.get("volume_mode") == "MANUAL" else 1
        )
        self.manual_volume_knob.setValue(
            float(self.parameters.get("receiver_volume", 0) or 0)
        )
        self.receiver_diameter_knob.setValue(
            float(self.parameters.get("receiver_diameter", 0) or 0)
        )
        self.receiver_length_knob.setValue(
            float(self.parameters.get("receiver_length", 0) or 0)
        )
        self._apply_volume_mode_widgets(
            str(self.parameters.get("volume_mode", "MANUAL"))
        )
        ...
        self.cv_atmo_dia_knob.setValue(
            float(self.parameters.get("cv_atmo_dia", 0) or 0)
        )
        self.cv_tank_dia_knob.setValue(
            float(self.parameters.get("cv_tank_dia", 0) or 0)
        )
        self.relief_min_pressure_knob.setValue(
            float(self.parameters.get("relief_min_pressure", 0) or 0)
        )
        self.relief_stiff_pressure_knob.setValue(
            float(self.parameters.get("relief_stiff_pressure", 0) or 0)
        )
        self.relief_safety_pressure_knob.setValue(
            float(self.parameters.get("relief_safety_pressure", 0) or 0)
        )
        self.throttle_min_dia_knob.setValue(
            float(self.parameters.get("throttle_min_dia", 0) or 0)
        )
        self.throttle_stiff_dia_knob.setValue(
            float(self.parameters.get("throttle_stiff_dia", 0) or 0)
        )
        self.atmo_temp_knob.setValue(float(self.parameters.get("atmo_temp", 0) or 0))
        self.isothermal_radio.setChecked(
            str(self.parameters.get("thermo_mode", "ISOTHERMAL")) == "ISOTHERMAL"
        )
        self.adiabatic_radio.setChecked(
            str(self.parameters.get("thermo_mode", "ISOTHERMAL")) == "ADIABATIC"
        )
        self.master_isolation_check.setChecked(
            bool(self.parameters.get("master_isolation_open", False))
        )
    ...
    def get_parameters(self) -> dict[str, float | bool | str]:
        """Return current pneumatic configuration in UI units."""
    ...
    def _validate_system(self) -> None:
        ...
        if p_stiff and p_safe and p_stiff >= p_safe:
            errors.append(
                "Давление сброса жёсткости должно быть меньше аварийного сброса"
            )
        ...
        if th_min and (th_min < 0.2 or th_min > 10):
            warnings.append(
                "Диаметр минимального дросселя вне типового диапазона (0.2..10 мм)"
            )
        if th_stiff and (th_stiff < 0.2 or th_stiff > 10):
            warnings.append(
                "Диаметр дросселя жёсткости вне типового диапазона (0.2..10 мм)"
            )
