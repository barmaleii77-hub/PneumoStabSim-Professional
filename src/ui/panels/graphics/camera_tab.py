...
class CameraTab(QWidget):
 ...
 Signals:
 camera_changed (dict): Параметры камеры изменились
 """
 ...
 def _setup_ui(self) -> None:
  ...
  # Приводим активность контролов в соответствие с текущими чекбоксами
 ...
 def _build_camera_group(self) -> QGroupBox:
  ...
  auto_rotate.clicked.connect(lambda checked: (self._update_enabled_states(), self._emit()))
  ...
  manual.clicked.connect(lambda checked: (self._update_enabled_states(), self._emit()))
  self._controls["manual_camera"] = manual
  grid.addWidget(manual, r,0,1,2)
  r +=1

  camx = LabeledSlider("Камера X", -100_000.0,100_000.0,10.0, decimals=0, unit="мм")
  camx.valueChanged.connect(lambda v: self._emit())
  self._controls["camera_pos_x"] = camx
  grid.addWidget(camx, r,0,1,2)
  r +=1

  camy = LabeledSlider("Камера Y", -100_000.0,100_000.0,10.0, decimals=0, unit="мм")
  camy.valueChanged.connect(lambda v: self._emit())
  self._controls["camera_pos_y"] = camy
  grid.addWidget(camy, r,0,1,2)
  r +=1

  camz = LabeledSlider("Камера Z", -100_000.0,100_000.0,10.0, decimals=0, unit="мм")
 ...
 def _build_orbit_group(self) -> QGroupBox:
  ...
  yaw = LabeledSlider("Yaw", -99_999.0,99_999.0,1.0, decimals=0, unit="°")
  yaw.valueChanged.connect(lambda v: self._emit())
  self._controls["orbit_yaw"] = yaw
  grid.addWidget(yaw, r,0,1,2)
  r +=1

  pitch = LabeledSlider("Pitch", -99_999.0,99_999.0,1.0, decimals=0, unit="°")
  ...
  inertia_enabled.clicked.connect(lambda checked: (self._update_enabled_states(), self._emit()))
  ...
  rotate_smooth = LabeledSlider("Сглаживание вращения",0.0,1.0,0.01, decimals=2)
  ...
  pan_smooth = LabeledSlider("Сглаживание панорамирования",0.0,1.0,0.01, decimals=2)
 ...
 def _build_world_group(self) -> QGroupBox:
  ...
  wx = LabeledSlider("Позиция X", -100_000.0,100_000.0,10.0, decimals=0, unit="мм")
  wx.valueChanged.connect(lambda v: self._emit())
  self._controls["world_pos_x"] = wx
  grid.addWidget(wx, r,0,1,2)
  r +=1

  wy = LabeledSlider("Позиция Y", -100_000.0,100_000.0,10.0, decimals=0, unit="мм")
  wy.valueChanged.connect(lambda v: self._emit())
  self._controls["world_pos_y"] = wy
  grid.addWidget(wy, r,0,1,2)
  r +=1

  wz = LabeledSlider("Позиция Z", -100_000.0,100_000.0,10.0, decimals=0, unit="мм")
 ...
 def _apply_preset(self, name: str) -> None:
  ...
  # Синхронизируем доступность контролов после пресета
  self._update_enabled_states()
 ...
 def _update_enabled_states(self) -> None:
  """Активирует/деактивирует связанные контролы в зависимости от чекбоксов.

 - `auto_rotate_speed` активно только при включенном `auto_rotate`
 - Ручные параметры камеры (позиция/углы) активны только при `manual_camera`
 - `orbit_inertia` активно только при включенной инерции
 """
 try:
  auto_rotate = bool(self._controls.get("auto_rotate").isChecked()) if self._controls.get("auto_rotate") else False
  if self._controls.get("auto_rotate_speed"):
   self._controls["auto_rotate_speed"].set_enabled(auto_rotate)

  manual = bool(self._controls.get("manual_camera").isChecked()) if self._controls.get("manual_camera") else False
  for k in ("camera_pos_x","camera_pos_y","camera_pos_z","camera_rot_x","camera_rot_y","camera_rot_z"):
   if self._controls.get(k):
    self._controls[k].set_enabled(manual)

  inertia_on = bool(self._controls.get("orbit_inertia_enabled").isChecked()) if self._controls.get("orbit_inertia_enabled") else False
