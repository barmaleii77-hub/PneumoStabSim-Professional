...
def _empty_payload(*, seed: str = "generated") -> dict[str, Any]:
    ...
    return {
        "metadata": {
            ...
            "last_migration": "",
            "migrated_from": "",
            "migration_date": timestamp.split("T", maxsplit=1)[0],
            "total_parameters": 0,
            "description": "Autogenerated defaults; no settings file available.",
...
def load_default_settings_payload() -> dict[str, Any]:
    ...
    metadata.setdefault("version", fallback["metadata"]["version"])
    metadata.setdefault("schema_version", fallback["metadata"]["schema_version"])
    metadata.setdefault("migrations", fallback["metadata"]["migrations"])
    metadata.setdefault("last_migration", fallback["metadata"]["last_migration"])
    metadata.setdefault("migrated_from", fallback["metadata"]["migrated_from"])
    metadata.setdefault("migration_date", fallback["metadata"]["migration_date"])
    metadata.setdefault("total_parameters", fallback["metadata"]["total_parameters"])
    metadata.setdefault("description", fallback["metadata"]["description"])
    metadata.setdefault(
        "operational_imperatives", fallback["metadata"]["operational_imperatives"]
    )
