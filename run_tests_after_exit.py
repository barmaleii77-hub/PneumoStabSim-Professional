...
Запускает тестовые скрипты после закрытия основного приложения,
формирует сводный отчёт и обеспечивает корректную настройку окружения Qt.
"""
...
class PostExitTestRunner:
    ...
    def run_all_tests(self, scripts: Optional[List[Path]] = None) -> Dict:
        """Запускает все тестовые скрипты и возвращает агрегированный результат."""
    ...
    def save_report(self, output_file: Optional[Path] = None) -> None:
        """Сохраняет отчёт о тестах в Markdown-файл."""
        ...
        for result in self.test_results:
            ...
            report_lines.extend(
                [
                    ...
                    "**STDOUT:**",
                    "```",
                    result["stdout"][:500],  # Первые 500 символов
                    "```",
                    "",
                ]
            )
        ...
        print(f"💾 Отчёт сохранен: {output_file}")
