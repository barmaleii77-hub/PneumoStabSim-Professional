...
class TerminalDiagnostic:
    ...
    def test_python_version(self):
        ...
        if version < (3, 8):
            self.log_result("PYTHON", "ERROR", f"Version {version_str} too old (need 3.8+)")
            return False
        elif version >= (3, 12):
            self.log_result("PYTHON", "WARNING", f"Version {version_str} very new (recommend 3.8-3.11)")
            return True
        else:
    ...
    def test_encoding_support(self):
        ...
        try:
            ...
            if default_encoding.lower() in ['utf-8', 'utf8']:
                self.log_result("ENCODING", "OK", f"Default encoding: {default_encoding}")
            else:
                self.log_result("ENCODING", "WARNING", f"Default encoding: {default_encoding} (not UTF-8)")
            
            # Test locale encoding
            try:
                locale_encoding = locale.getpreferredencoding()
                if 'utf' in locale_encoding.lower():
                    self.log_result("LOCALE", "OK", f"Locale encoding: {locale_encoding}")
                else:
                    self.log_result("LOCALE", "WARNING", f"Locale encoding: {locale_encoding} (not UTF-8)")
            except Exception as e:
                self.log_result("LOCALE", "ERROR", f"Cannot detect locale encoding: {e}")
    ...
    def test_terminal_capabilities(self):
        ...
        try:
            ...
            if hasattr(sys.stdout, 'isatty') and sys.stdout.isatty():
                ...
                self.log_result("TERMINAL", "WARNING", "Not running in interactive terminal")
                ...
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        if '65001' in codepage:
                            self.log_result("CODEPAGE", "OK", "Windows codepage: UTF-8 (65001)")
                        else:
                            self.log_result("CODEPAGE", "WARNING", f"Windows codepage: {codepage} (not UTF-8)")
                    else:
                        self.log_result("CODEPAGE", "WARNING", "Cannot detect Windows codepage")
                except Exception as e:
                    self.log_result("CODEPAGE", "WARNING", f"Codepage detection failed: {e}")
            
            # Test ANSI color support
            try:
                # Try to print ANSI colors
                ansi_test = "\033[31mRed\033[0m \033[32mGreen\033[0m \033[34mBlue\033[0m"
                if os.getenv('TERM'):
                    self.log_result("ANSI", "OK", f"TERM environment variable: {os.getenv('TERM')}")
                else:
                    self.log_result("ANSI", "WARNING", "TERM environment variable not set")
                    
            except Exception as e:
                self.log_result("ANSI", "WARNING", f"ANSI color test failed: {e}")
                
        except Exception as e:
            self.log_result("TERMINAL", "ERROR", f"Terminal capability test failed: {e}")
    ...
    def test_qt_imports(self):
        ...
        try:
            ...
            try:
                ...
                self.log_result("QT", "WARNING", f"PySide6 not available, PyQt6 {version} found")
                return True
            except ImportError:
                self.log_result("QT", "ERROR", "No Qt framework available (PySide6 or PyQt6)")
                return False
    ...
    def generate_fixes(self):
        ...
        if any("Unicode" in error for error in self.errors):
            fixes.append("Set environment variables: PYTHONIOENCODING=utf-8 PYTHONUTF8=1")
