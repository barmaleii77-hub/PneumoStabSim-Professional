...
def check_update_functions():
    ...
    update_functions = {
        'applyEffectsUpdates': ['bloomThreshold', 'ssaoRadius', 'dofFocusDistance', 'tonemapMode'],
...
def main():
    ...
    for param, result in python_results.items():
        ...
        print(f"{status} {param:20s} - Словарь:{result['in_dict']} | UI:{result['has_ui']} | Handler:{result['has_handler']} | Emit:{result['in_emit']}")
    ...
    for param, result in qml_prop_results.items():
        ...
        print(f"{status} {param:20s} - Объявлен: {result['declared']:5} | Тип: {prop_type}")
    ...
    for param, result in binding_results.items():
        ...
        bindings = ", ".join(result['bound_to']) if result['bound_to'] else "НЕ ПРИВЯЗАН К РЕНДЕРИНГУ!"
    ...
    for param, info in CRITICAL_PARAMS.items():
        ...
        if not (python_ok and qml_declared and is_bound and is_updated):
            broken_params.append((param, info['description'], {
                'python': python_ok,
    ...
    if broken_params:
        ...
        for param, desc, status in broken_params:
            ...
            print(f"   Привязка:      {'✅' if status['binding'] else '❌ НЕ ПРИВЯЗАН К CANVAS!'}")
            print(f"   Update func:   {'✅' if status['update'] else '❌'}")
            print()
        
        print("💡 РЕШЕНИЕ:")
        print("   Нужно добавить привязки в ExtendedSceneEnvironment/PrincipledMaterial/Light")
        print("=" * 80)
    else:
