...
# -*- coding: utf-8 -*-
"""
ANIMATED Z-axis cylindrical joints test - checking animation with correct joints
"""
import sys
import os
...
class AnimatedZAxisWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        
        self.setWindowTitle("ANIMATED Z-AXIS JOINTS - ANIMATION TEST")
        self.resize(1400, 900)
        
        self.qml_widget = QQuickWidget(self)
        self.qml_widget.setResizeMode(QQuickWidget.SizeRootObjectToView)
        
        qml_path = Path("animated_z_axis.qml")
        qml_path.write_text(ANIMATED_QML, encoding='utf-8')
        ...
        if self.qml_widget.status() == QQuickWidget.Status.Error:
            ...
            print("? Animated Z-axis joints loaded successfully")
        
        self.setCentralWidget(self.qml_widget)

def main():
    print("="*60)
    print("ANIMATED Z-AXIS JOINTS - ANIMATION TEST")
    print("="*60)
    print("FEATURES:")
    print("? Levers rotate with sine wave animation")
    print("? Pistons move inside very transparent cylinders")
    print("? Rods extend/retract from pistons correctly")
    print("? All joints oriented along Z-axis")
    print("? Joints animate slightly with suspension motion")
    print("="*60)
    
    app = QApplication(sys.argv)
    window = AnimatedZAxisWindow()
    window.show()
    sys.exit(app.exec())
