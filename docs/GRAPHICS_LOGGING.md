...
- `FileCyclerWidget` дополнительно предупреждает о пропавшем файле: в лог попадает строка `Файл не найден: …`, UI подсвечивает красный бейдж `⚠ файл не найден`, а при активном окне появится модальный диалог с путём. Используйте этот сигнал, чтобы убедиться, что пользователь действительно положил HDR/текстуры в доступный каталог. 【F:src/ui/panels/graphics/widgets.py†L486-L576】
