...
- Новые тесты должны выполняться на всех поддерживаемых платформах. Не помечайте их `skip`/`xfail` без документированного исключения; вместо этого фиксируйте недостающие зависимости через `tools.cross_platform_test_prep` и указывайте причину в `CI_SKIP_REASON`, если пропуски временно признаны в CI. CI дополнительно публикует `reports/tests/skipped_tests_summary.md` и требует `PSS_ALLOW_SKIPPED_TESTS=1` вместе с `CI_SKIP_REASON`, если нужны временные пропуски.
