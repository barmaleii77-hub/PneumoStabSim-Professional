...
2. Подготавливает Python 3.13, устанавливает `uv`, dev-зависимости и Qt 6.10.1 (вместе с плагинами `qtquick3d`, `qtshadertools`, `qtimageformats`). Скрипт `tools/setup_qt.py` по умолчанию разворачивает именно версию **6.10.1**, чтобы избежать расхождений между локальной средой, CI и документацией.
