...
- `pytest_targets.txt` предназначен только для добавления временных дополнительных путей. Базовые цели (`tests/unit`, `tests/integration`, `tests/ui`) всегда выполняются первыми и не могут быть отключены; файл не должен использоваться для сужения покрытия тестов.
