...
- `pytest_targets.txt` удалён из репозитория, чтобы не создавалось впечатления, будто он может сужать прогон. Дополнительные критические сценарии подключайте через `pytest_unit_targets.txt`, `pytest_integration_targets.txt`, `pytest_ui_targets.txt` или переменные окружения `PYTEST_*_TARGETS`; базовый набор всегда охватывает весь каталог `tests/` и отдельные задачи `test-unit`, `test-integration`, `test-ui` дополнительно проверяют неожиданно пропущенные тесты по итоговым JUnit-отчётам.
- Новые тесты должны выполняться на всех поддерживаемых платформах. Не помечайте их `skip`/`xfail` без документированного исключения; вместо этого фиксируйте недостающие зависимости через `tools.cross_platform_test_prep` и указывайте причину в `CI_SKIP_REASON`, если пропуски временно признаны в CI. Допустимые пропуски должны содержать маркер `pytest-skip-ok` в сообщении (`reason`), иначе сборка упадёт при анализе результатов. CI дополнительно публикует `reports/tests/skipped_tests_summary.md` и требует `PSS_ALLOW_SKIPPED_TESTS=1` вместе с `CI_SKIP_REASON`, если нужны временные пропуски.
