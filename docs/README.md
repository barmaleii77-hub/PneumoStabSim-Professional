# PneumoStabSim Professional v4.9.9 — Documentation Hub
...
- **Каталог и манифест** — рабочие HDRI хранятся в `assets/hdr/` и описаны в
  ...
  `python -m tools.task_runner verify-hdr-assets --fetch-missing` — она синхронизирует локальный кэш, проверяет SHA-256 и валидирует
  Radiance-заголовки перед запуском UI. Итоговый инвентарь с лицензиями и
  назначениями опубликован в `assets/hdr/README.md`.
...
> Эта шпаргалка поддерживается в актуальном состоянии для релиза v4.9.9. Обновляйте её при изменении архитектуры, tooling или путей к ключевым документам.
