...
- **Нормализация путей и fallback** — панели используют
  ...
  `src/ui/panels/graphics/environment_tab.py` и проверяется юнитами
  `tests/unit/ui/test_hdr_discovery.py` и `tests/unit/ui/test_main_window_hdr_paths.py`.
...
- **Устойчивость настроек** — выбранная HDR-карта и параметры bloom
  ...
  через `SettingsManager`, восстанавливая фон, интенсивность bloom и назначенные
  текстуры без ручной перенастройки.
