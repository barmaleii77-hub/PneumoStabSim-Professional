...
**Статус готовности:** ✅ Готово к объединению (ожидает внешнего подтверждения)
...
1. **Восстановить зелёное состояние CI** — устранить упомянутые провалы unit-тестов (`structlog`, kinematics) до релиза.  
2. **Увеличить покрытие** — текущий показатель 42.28%; рекомендуется усилить тесты освещения и настроек.  
3. **Закрыть предупреждения `qmllint`** — закрепить фиксы в дополнительных MR/PR согласно Phase 3 playbook.  
4. **Зафиксировать регулярность HDR-валидации** — `make verify hdr-verify` и `validate_hdr_orientation` должны запускаться перед сборками и синхронизироваться с обновлёнными журналами `logs/ibl/ibl_events.jsonl`.
...
- ✅ **Следующий шаг:** получить подтверждение от перечисленных ролей перед выполнением `merge` в целевую ветку.
