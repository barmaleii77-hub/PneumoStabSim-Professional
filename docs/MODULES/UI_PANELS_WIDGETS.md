...
- В новых проектах рекомендуется переходить на QML `SimulationPanel`. Документ
  ...
  старых макросов.
- QML `SimulationPanel` экспонирует переключатели кинематика/динамика,
  изотермика/адиабатика, проверки пересечений и отдельные флажки учёта
  пружин/демпферов в кинематике. Панель включает пресеты дорожных профилей с
  полем пользовательского пути и ввод температуры окружающей среды; значения
  сразу уходят в `SettingsManager`, синхронно обновляя категории `modes` и
  `pneumatic` для сохранения выбранного терморежима и условий среды. 【F:assets/qml/Panels/SimulationPanel.qml†L18-L188】【F:assets/qml/Panels/SimulationPanel.qml†L630-L750】
