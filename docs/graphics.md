...
- При необходимости можно переопределить исполняемый файл Qt Shader Baker переменной окружения `QSB_COMMAND`
  (например, `QSB_COMMAND="/opt/qt/bin/qsb" make check-shaders`).
- Для подготовки `.qsb` к релизу выполните `make shader-artifacts` (или `python tools/validate_shaders.py --emit-qsb`).
  В этом режиме файлы и логи размещаются в `reports/shaders/`, при этом каталог игнорируется Git и подходит для
  публикации в CI артефактах.
