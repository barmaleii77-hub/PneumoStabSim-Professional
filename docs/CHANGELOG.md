...
## [2.1.0] - 2027-01-18
### Added
- Версии приложения, настроек и модульных окон синхронизированы с релизом 2.1.0
  в `pyproject.toml`, `config/app_settings.json` и `src/ui` пакетах.
- Обновлены пользовательские гайды (README, hub документации, HDR-руководства)
  с актуальными ссылками на HDR-поток и контроль версий.

### Changed
- CHANGELOG расширен свежим блоком HDR-ветки: отдельные ползунки `HDR Maximum`
  и `HDR Scale`, нормализация путей и предупреждения при отсутствии skybox.
- Release readiness и quality matrix отражают новую целевую версию и артефакты
  релиза 2.1.0.
