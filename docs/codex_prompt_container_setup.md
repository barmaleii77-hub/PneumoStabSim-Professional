...
• Python 3.13.x, PySide6/Qt 6.10.0 (как в логах запуска и в `tools/setup_qt.py`).
• Qt CLI-утилиты в контейнере: qmllint, qsb (через aqtinstall Qt 6.10.0 — та же версия, что использует CI).
...
• Параметры запуска Qt: тесты в OpenGL и Vulkan (QSG_RHI_BACKEND=opengl / QSG_RHI_BACKEND=vulkan), QT_QUICK_CONTРОLS_STYLE=Fusion, headless-профиль Qt (QT_QPA_PLATFORM=offscreen, QT_QUICK_BACKEND=software) при отсутствии DISPLAY и VK_ICD_FILENAMES для Lavapipe. Эти же дефолты прописаны в env.sample и применяются configure_qt_environment() — см. `docs/ENVIRONMENT_SETUP.md` для актуального списка.
