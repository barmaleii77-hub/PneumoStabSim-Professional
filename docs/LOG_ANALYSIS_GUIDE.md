...
**Проблема:** HDR источники не резолвятся или включается fallback

**Решение:**
1. Откройте `logs/ibl/ibl_events.jsonl` и убедитесь, что события помечены как `status=ok`; `missing`, `remote` или `empty` означают, что путь не найден/указан удалённый ресурс/пустое значение.
2. Проверьте, что путь указан в поддерживаемых каталогах (`assets/hdr/`, `assets/hdri/`, `assets/qml/assets/`) и соответствует манифесту `assets/hdr/hdr_manifest.json`.
3. Если путь задан вручную, повторно запустите `make verify hdr-verify` или модульные тесты `tests/unit/ui/test_main_window_hdr_paths.py`, чтобы убедиться, что `normalizeHdrPath` возвращает корректный `file://` URL и UI снимает предупреждение `⚠ файл не найден`.

```bash
# Проверка наличия файлов и событий IBL
ls -la assets/hdr/
tail -n 20 logs/ibl/ibl_events.jsonl
```
