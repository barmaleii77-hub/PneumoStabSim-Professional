...
def diff_dict(
    ...
    """Рекурсивно сравнивает JSON-подобные структуры и возвращает отличия."""
...
def build_report(result: AuditResult, config_path: Path, baseline_path: Path) -> str:
    ...
    schema_section: list[str] = ["### Проверка схемы"]
    ...
    diff_section: list[str] = ["### Сравнение с эталоном"]
    if result.diffs:
        diff_section.append(f"- ⚠️ Найдено расхождений: {len(result.diffs)}")
        for entry in result.diffs[:20]:
            diff_section.append(
                "  - "
                f"`{entry.path}`: эталон = `{entry.baseline}` → текущий = `{entry.current}`"
            )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.strict and result.expected_hash and result.expected_hash != result.actual_hash:
        exit_code = 1
