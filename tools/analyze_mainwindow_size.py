...
def analyze_mainwindow_module():
    ...
    if original_file.exists():
        ...
        print(f"Original file: src/ui/main_window.py")
        print(f"{'main_window.py (ORIGINAL)':45} {orig_lines:5} lines")
        print()
        
        coordinator_lines = len((base_dir / "main_window_refactored.py").read_text(encoding="utf-8").splitlines())
        reduction = round((1 - coordinator_lines / orig_lines) * 100, 1)
        
        print(f"✅ Coordinator reduction: -{reduction}% ({orig_lines} → {coordinator_lines} lines)")
