...
def check_geometry_status():
    ...
    # Проверка файлов
    geo_files = ['src/ui/panels/panel_geometry.py', 'src/ui/geo_state.py', 'src/ui/geo_state_new.py']
    ...
    if panel_geo.exists():
        try:
            ...
            status['unified_params'] = all(param in content for param in required_params)
...
def check_receiver_status():
    ...
    if receiver_py.exists():
        try:
            ...
            status['thermodynamics'] = 'isothermal' in content and 'adiabatic' in content
        except Exception as e:
...
def create_report(audit_data):
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f"- **Унифицированные параметры:** {'✅' if geo['unified_params'] else '❌'}\n")
        f.write(f"- **СИ единицы (м, шаг 0.001, decimals=3):** {'✅' if geo['si_units'] else '❌'}\n")
        f.write("\n")
        ...
        f.write(f"- **Термодинамика (изотерма/адиабата):** {'✅' if recv['thermodynamics'] else '❌'}\n")
        f.write("\n")
...
def main():
    ...
    print(f"📁 Структура: {sum(1 for info in audit_data['structure'].values() if info['exists'])}/{len(audit_data['structure'])} директорий")
    print(f"📄 Ключевые файлы: {sum(1 for info in audit_data['files'].values() if info['exists'])}/{len(audit_data['files'])}")
    print(f"🔧 Геометрия готова: {'✅' if audit_data['geometry']['unified_params'] else '❌'}")
