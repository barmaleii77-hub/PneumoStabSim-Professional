    ...
    for root, dirs, files in os.walk(directory):
        ...
        for file in files:
            if any(file.endswith(ext) for ext in extensions):
                ...
                stats.append(FileStats(path=file_path, lines=lines, size_kb=size_kb))
...
def print_report(stats: List[FileStats], top_n: int = 20):
    ...
    for i, stat in enumerate(stats[:top_n], 1):
        print(
            f"{i:<4} {stat.lines:<7} {stat.size_kb:<10.2f} {stat.priority:<20} {stat.relative_path:<60}"
        )
    ...
    if critical > 0:
        print(
            f"  ⚠️  Найдено {critical} критических файлов - СРОЧНО требуется рефакторинг!"
        )
    if high > 0:
        print(
            f"  ⚠️  Найдено {high} файлов высокого приоритета - рекомендуется рефакторинг"
        )
    if medium > 0:
        print(
            f"  ℹ️  Найдено {medium} файлов среднего приоритета - рассмотреть рефакторинг"
        )
