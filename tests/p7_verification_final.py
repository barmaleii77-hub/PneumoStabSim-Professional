...
def test_p7_components():
    ...
    try:
        ...
        assert stats['dropped_count'] == 4, f"Expected 4 dropped, got {stats['dropped_count']}"
        
        print(f"   ? LatestOnlyQueue: Keeps latest frame, dropped {stats['dropped_count']}/5")
    ...
    try:
        ...
        print(f"   ? TimingAccumulator: {steps} steps for ~5ms (fixed timestep working)")
    ...
    try:
        ...
        print(f"   ? PerformanceMetrics: avg={avg_fps:.1f} FPS, target={target_fps:.1f} FPS")
    ...
    try:
        ...
        assert hasattr(bus, 'start_simulation'), "StateBus should have start_simulation signal"
