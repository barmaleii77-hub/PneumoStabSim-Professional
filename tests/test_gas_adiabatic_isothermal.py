...
from src.pneumo.gas_state import (
    LineGasState, TankGasState, iso_update, adiabatic_update, 
    apply_instant_volume_change, p_from_mTV
)
...
class TestIsothermalProcess(unittest.TestCase):
    ...
    def test_isothermal_compression(self):
        ...
        self.assertAlmostEqual(self.line_state.p, expected_p, delta=1000)  # 1000 Pa tolerance
...
class TestAdiabaticProcess(unittest.TestCase):
    ...
    def test_adiabatic_invariants(self):
        ...
        initial_TV_gamma_minus_1 = self.line_state.T * (self.line_state.V_curr ** (GAMMA_AIR - 1.0))
        ...
        for volume_ratio in [0.5, 0.7, 1.3, 2.0]:
            ...
            new_TV_gamma_minus_1 = self.line_state.T * (self.line_state.V_curr ** (GAMMA_AIR - 1.0))
            self.assertAlmostEqual(new_TV_gamma_minus_1 / initial_TV_gamma_minus_1, 1.0, places=5)
    ...
    def test_ideal_gas_consistency(self):
        ...
        expected_p = p_from_mTV(self.line_state.m, self.line_state.T, self.line_state.V_curr)
...
class TestReceiverVolumeChange(unittest.TestCase):
    ...
    def test_mass_consistency(self):
        ...
        initial_mass_calculated = (self.tank_adiabatic.p * self.tank_adiabatic.V) / (R_AIR * self.tank_adiabatic.T)
        ...
        final_mass_calculated = (self.tank_adiabatic.p * self.tank_adiabatic.V) / (R_AIR * self.tank_adiabatic.T)
...
class TestGasStateValidation(unittest.TestCase):
    ...
    def test_edge_cases(self):
        ...
        line_state = LineGasState(Line.A1, m=1e-10, T=300, p=1000, V_prev=1e-6, V_curr=1e-6)
