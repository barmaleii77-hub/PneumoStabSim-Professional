...
def test_three_d_scene_emits_rollback_signals(qapp) -> None:  # type: ignore[missing-type-doc]
    engine = QQmlEngine()
    component = QQmlComponent(engine)
    component.loadUrl(QUrl.fromLocalFile(str(Path("qml/ThreeDScene.qml").resolve())))

    if component.status() != QQmlComponent.Ready:
        messages = component.errorString()
        raise RuntimeError(f"Failed to load ThreeDScene.qml: {messages}")
    ...
    try:
        ...
        root.setProperty(
            ...
            {"undoPostEffects": True, "resetSharedMaterials": True},
        )
        qapp.processEvents()
