...
MODULE_PATH = (
    Path(__file__).resolve().parents[2] / "src" / "ui" / "main_window" / "qml_bridge.py"
)
...
def test_flush_updates_notifies_status_bar_on_failure(monkeypatch, event_logger):
    ...
    assert any(
        "QML не принял пакет обновлений" in message for message in status_messages
    )
...
def test_set_simulation_state_failure_logs_and_notifies(event_logger):
    ...
    assert status_messages
    assert any(
        "QML не принял состояние симуляции" in message for message in status_messages
    )
