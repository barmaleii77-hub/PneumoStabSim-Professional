...
def test_get_3d_geometry_update_returns_meters() -> None:
    ...
    # Millimetre duplicates for legacy consumers
    assert update["frame_length_mm"] == pytest.approx(3200.0)
    assert update["frame_height_mm"] == pytest.approx(650.0)
    assert update["frame_beam_size_mm"] == pytest.approx(120.0)
    assert update["lever_length_mm"] == pytest.approx(800.0)
    assert update["cylinder_body_length_mm"] == pytest.approx(500.0)
    assert update["tail_rod_length_mm"] == pytest.approx(100.0)
    assert update["track_width_mm"] == pytest.approx(1600.0)
    assert update["frame_to_pivot_mm"] == pytest.approx(600.0)
    assert update["cyl_diam_mm"] == pytest.approx(80.0)
    assert update["stroke_mm"] == pytest.approx(300.0)
    assert update["dead_gap_mm"] == pytest.approx(5.0)
    assert update["rod_diameter_mm"] == pytest.approx(35.0)
    assert update["piston_rod_length_mm"] == pytest.approx(200.0)
    assert update["piston_thickness_mm"] == pytest.approx(25.0)
...
def test_get_3d_geometry_update_respects_state_overrides() -> None:
    ...
    assert update["frame_length_mm"] == pytest.approx(2750.0)
    assert update["frame_height_mm"] == pytest.approx(700.0)
    assert update["frame_beam_size_mm"] == pytest.approx(150.0)
    assert update["lever_length_mm"] == pytest.approx(950.0)
    assert update["cylinder_body_length_mm"] == pytest.approx(450.0)
    assert update["tail_rod_length_mm"] == pytest.approx(120.0)
    assert update["track_width_mm"] == pytest.approx(1720.0)
    assert update["frame_to_pivot_mm"] == pytest.approx(550.0)
    assert update["cyl_diam_mm"] == pytest.approx(90.0)
    assert update["stroke_mm"] == pytest.approx(320.0)
    assert update["dead_gap_mm"] == pytest.approx(4.0)
    assert update["rod_diameter_mm"] == pytest.approx(28.0)
    assert update["piston_rod_length_mm"] == pytest.approx(240.0)
    assert update["piston_thickness_mm"] == pytest.approx(18.0)
