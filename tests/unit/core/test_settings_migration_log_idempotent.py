...
def test_idempotent_run_does_not_duplicate_events(tmp_path: Path) -> None:
    ...
    settings_path.write_text(
        json.dumps({"metadata": {}, "current": {}, "defaults_snapshot": {}}, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    ...
    exit_code1 = settings_migrate.main([
        ...
        "--in-place", "--log-dir", str(log_root),
    ])
    assert exit_code1 == 0
    log_file = log_root / "migrations.jsonl"
    assert log_file.exists()
    events_first = _read_events(log_file)
    migration_events_first = [e for e in events_first if e.get("migration") == "001_add_flag"]
    final_events_first = [e for e in events_first if e.get("event") == "migration-run-complete"]
    ...
    migration_events_second = [e for e in events_second if e.get("migration") == "001_add_flag"]
    final_events_second = [e for e in events_second if e.get("event") == "migration-run-complete"]
