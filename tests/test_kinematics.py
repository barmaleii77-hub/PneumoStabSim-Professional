...
from src.core.geometry import Point2, Segment2, Capsule2
...
class TestTrackInvariant(unittest.TestCase):
    ...
    def test_mirrored_sides(self):
        ...
        self.assertTrue(
            self.validator.validate_track_invariant(track_right, arm_right, pivot_right)
        )
        self.assertTrue(
            self.validator.validate_track_invariant(track_left, arm_left, pivot_left)
        )
...
class TestStrokeValidation(unittest.TestCase):
    ...
    def test_residual_volume_minimum(self):
        ...
        # Sufficient residual
        self.assertTrue(
            validator.validate_residual_volume(min_required, diameter, stroke)
        )

        # Insufficient residual
        self.assertFalse(
            validator.validate_residual_volume(min_required * 0.3, diameter, stroke)
        )
...
class TestAngleStrokeRelationship(unittest.TestCase):
    ...
    def test_symmetric_angles(self):
        """Test �y ? �? symmetry"""
        lever_kin = LeverKinematics(
            arm_length=0.4, pivot_position=Point2(0.0, 0.0), pivot_offset_from_frame=0.3
        )
