...
from physics.odes import RigidBody3DOF, f_rhs, create_initial_conditions, validate_state
from physics.forces import project_forces_to_vertical_and_moments, create_test_suspension_state
from physics.integrator import step_dynamics, create_default_rigid_body, PhysicsLoop, PhysicsLoopConfig
...
def test_force_projection():
    ...
    for wheel_name in ['LP', 'PP', 'LZ', 'PZ']:
        suspension_states[wheel_name] = create_test_suspension_state(wheel_name, F_cyl=test_force)
    ...
    print(f"PASS: Equal forces produce zero moments")
    ...
    suspension_states['PP']['F_cyl_axis'] = test_force * 1.5  # More force at front right
