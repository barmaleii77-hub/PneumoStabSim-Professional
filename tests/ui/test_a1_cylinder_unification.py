...
def test_unified_cylinder_parameters():
    ...
    for param in old_params:
        assert (
            param not in panel.parameters
        ), f"Old parameter {param} should not exist in defaults"
...
def test_si_units_and_precision():
    ...
    for (
        ...
        expected_step,
        expected_decimals,
    ) in controls_to_check:
        control = getattr(panel, control_name)

        # Check units (assuming RangeSlider has units property)
        if hasattr(control, "units"):
            assert (
                control.units == expected_units
            ), f"{control_name} should have units '{expected_units}'"

        # Check step (assuming step property exists)
        if hasattr(control, "step"):
            assert (
                abs(control.step - expected_step) < 1e-6
            ), f"{control_name} should have step {expected_step}"

        # Check decimals
        if hasattr(control, "decimals"):
            assert (
                control.decimals == expected_decimals
            ), f"{control_name} should have {expected_decimals} decimals"
...
def test_parameter_ranges():
    ...
    for param, (min_val, max_val, default_val) in expected_ranges.items():
        ...
        assert (
            actual_default == default_val
        ), f"Parameter {param} should have default {default_val}, got {actual_default}"

        # Check parameter is within expected range
        assert (
            min_val <= actual_default <= max_val
        ), f"Parameter {param} default should be in range [{min_val}, {max_val}]"
