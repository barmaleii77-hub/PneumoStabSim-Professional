...
print(
    f"   rod_diameter_slider:         {not hasattr(panel, 'rod_diameter_slider')} (dolzhno byt' True)"
)
print(
    f"   piston_rod_length_slider:    {not hasattr(panel, 'piston_rod_length_slider')} (dolzhno byt' True)"
)
print(
    f"   piston_thickness_slider:     {not hasattr(panel, 'piston_thickness_slider')} (dolzhno byt' True)"
)
...
print(
    f"   'rod_diameter_m' v parameters:     {('rod_diameter_m' in params)} = {params.get('rod_diameter_m', 'NE NAYDENO')}"
)
print(
    f"   'piston_rod_length_m' v parameters: {('piston_rod_length_m' in params)} = {params.get('piston_rod_length_m', 'NE NAYDENO')}"
)
print(
    f"   'piston_thickness_m' v parameters:  {('piston_thickness_m' in params)} = {params.get('piston_thickness_m', 'NE NAYDENO')}"
)
print(
    f"   'rod_diameter' OTSUTSTVUET:         {('rod_diameter' not in params)} (dolzhno byt' True)"
)
print(
    f"   'piston_rod_length' OTSUTSTVUET:    {('piston_rod_length' not in params)} (dolzhno byt' True)"
)
print(
    f"   'piston_thickness' OTSUTSTVUET:     {('piston_thickness' not in params)} (dolzhno byt' True)"
)
