...
class TestGeometryPanelFunctionality:
    ...
    def test_si_unit_ranges_displayed(self, geometry_panel):
        ...
        assert geometry_panel.wheelbase_range_label.text() == "3.0 - 4.5 м", \
            "Wheelbase range should be '3.0 - 4.5 м'"
        assert geometry_panel.track_range_label.text() == "1.4 - 2.0 м", \
            "Track range should be '1.4 - 2.0 м'"
        assert geometry_panel.lever_length_range_label.text() == "0.8 - 1.2 м", \
            "Lever length range should be '0.8 - 1.2 м'"
...
class TestPneumoPanelFunctionality:
    ...
    def test_si_unit_ranges_displayed(self, pneumo_panel):
        ...
        # Check SI unit values are correct
        assert pneumo_panel.relief_min_pressure_range_label.text() == "0.0 - 10.0 бар", \
            "Relief min pressure range should be '0.0 - 10.0 бар'"
        assert pneumo_panel.atmo_temp_range_label.text() == "-40.0 - 85.0 °C", \
            "Atmospheric temperature range should be '-40.0 - 85.0 °C'"
