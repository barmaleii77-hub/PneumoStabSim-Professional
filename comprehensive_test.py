...
class ComprehensiveTestSuite:
    ...
    def test_python_environment(self) -> bool:
        ...
        try:
            ...
            if version < (3, 8):
                self.log_result("Python Version", False, f"Python {version.major}.{version.minor}.{version.micro} < 3.8")
                return False
            else:
    ...
    def test_file_structure(self) -> bool:
        ...
        for file_path in optional_files:
            ...
            if path.exists():
                size = path.stat().st_size if path.is_file() else "DIR"
                self.log_result(f"Optional {file_path}", True, f"Available, size: {size}")
            else:
        ...
        if missing_critical:
            self.log_result("File Structure", False, f"Missing critical files: {missing_critical}", duration)
            return False
        else:
            self.log_result("File Structure", True, "All critical files present", duration)
            return True
    ...
    def test_qml_syntax(self) -> bool:
        ...
        try:
            for qml_file in qml_files:
                ...
                if brace_count != 0:
                    self.log_result(f"QML {qml_file}", False, f"Unmatched braces: {brace_count}")
                    continue
    ...
    def test_python_imports(self) -> bool:
        ...
        try:
            ...
            duration = time.time() - start
            self.log_result("Python Imports", True, "All modules imported successfully", duration)
            return True
    ...
    def test_graphics_panel_config(self) -> bool:
        ...
        try:
            ...
            self.log_result("Graphics Config", True, f"Test config: {len(test_config)} parameters")
            
            # Проверка критических параметров
            critical_params = ['key_brightness', 'glass_ior', 'ibl_enabled']
            for param in critical_params:
                if param in test_config:
                    self.log_result(f"Graphics Param {param}", True, f"Value: {test_config[param]}")
                else:
                    self.log_result(f"Graphics Param {param}", False, "Missing")
                    return False
            
            duration = time.time() - start
            self.log_result("Graphics Panel Config", True, "Configuration valid", duration)
            return True
    ...
    def test_ibl_system(self) -> bool:
        ...
        try:
            ...
            if not ibl_loader_path.exists():
                self.log_result("IBL Loader File", False, "IblProbeLoader.qml not found")
                return False
            ...
            for check_name, result in ibl_checks:
                self.log_result(f"IBL {check_name}", result, "Present" if result else "Missing")
                if not result:
                    return False
            ...
            for hdr_path in hdr_paths:
                ...
                if path.exists():
                    if hdr_path.endswith('.hdr'):
                        ...
                        self.log_result(f"HDR File {hdr_path}", True, f"Size: {size_mb:.1f}MB")
                        hdr_available = True
                    else:
                        self.log_result(f"HDR File {hdr_path}", True, "Documentation present")
                else:
            ...
            if main_qml_path.exists():
                ...
                for check_name, result in main_checks:
                    self.log_result(f"Main QML {check_name}", result, "Present" if result else "Missing")
        ...
        try:
            ...
            for global_name in globals_to_check:
                if hasattr(app, global_name):
                    ...
                    self.log_result(f"App Global {global_name}", True, f"Value: {value}")
                else:
            ...
            duration = time.time() - start
            self.log_result("Application Startup", True, "Core application structure OK", duration)
            return True
