...
class FixedAnglesWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        
        self.setWindowTitle("FIXED ANGLES SUSPENSION")
        self.resize(1400, 900)
        
        self.qml_widget = QQuickWidget(self)
        self.qml_widget.setResizeMode(QQuickWidget.SizeRootObjectToView)
        
        qml_path = Path("fixed_angles.qml")
        qml_path.write_text(FIXED_ANGLES_QML, encoding='utf-8')
        ...
        if self.qml_widget.status() == QQuickWidget.Status.Error:
            ...
            print("? Fixed angles loaded successfully")
        
        self.setCentralWidget(self.qml_widget)

def main():
    print("="*60)
    print("FIXED ANGLES SUSPENSION TEST")
    print("="*60)
    print("Testing corrected cylinder orientations")
    print("Should see cylinders pointing in logical directions")
    print("="*60)
    
    app = QApplication(sys.argv)
    window = FixedAnglesWindow()
    window.show()
    sys.exit(app.exec())
