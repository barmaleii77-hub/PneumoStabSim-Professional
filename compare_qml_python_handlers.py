...
def main():
    ...
    for py_prop in py_props.keys():
        ...
        camel_case_variants = [
            py_prop,
            ''.join(word.capitalize() if i > 0 else word for i, word in enumerate(py_prop.split('_')))
        ]
    ...
    update_functions = {f for f in qml_funcs if 'update' in f.lower() or 'apply' in f.lower()}
    ...
    if missing_in_qml:
        ...
        for prop in sorted(missing_in_qml)[:5]:  # Показываем первые 5
            snake_to_camel = ''.join(word.capitalize() if i > 0 else word for i, word in enumerate(prop.split('_')))
