...
def main():
    ...
    if "cameraInitialized" in qml_content and "Timer" in qml_content and "cameraInitTimer" in qml_content:
        ...
        camera_flag_pattern = r'(property bool autoRotate: false\s*property real autoRotateSpeed: 0\.5)'
        ...
        if re.search(camera_flag_pattern, qml_content):
            qml_content = re.sub(camera_flag_pattern, camera_flag_replacement, qml_content)
    ...
    Timer {
        ...
        if re.search(timer_pattern, qml_content, re.DOTALL):
            qml_content = re.sub(timer_pattern, timer_replacement, qml_content, flags=re.DOTALL)
        ...
        if re.search(behavior_pattern, qml_content, re.DOTALL):
            qml_content = re.sub(behavior_pattern, behavior_replacement, qml_content, flags=re.DOTALL)
        ...
        if re.search(pitch_behavior_pattern, qml_content, re.DOTALL):
            qml_content = re.sub(pitch_behavior_pattern, pitch_behavior_replacement, qml_content, flags=re.DOTALL)
    ...
    camera_check = (
        "cameraInitialized: false" in qml_check and
        "cameraInitTimer" in qml_check and
        "enabled: root.cameraInitialized" in qml_check
    )
    ...
    lighting_check = (
        "'keyLightBrightness'" in graphics_check and
        "'fillLightBrightness'" in graphics_check and
        "'pointLightBrightness'" in graphics_check
    )
