...
def check_main_qml():
    ...
    for prop, description in graphics_properties.items():
        if f"property" in content and prop in content:
    ...
    total_expected = len(integration_functions) + len(graphics_properties) + len(imports_3d) + len(components_3d)
...
def main():
    ...
    print(f"   python check_qml_graphics.py")
