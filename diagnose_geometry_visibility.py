...
def diagnose_geometry():
    ...
    print(f"🔴 PISTON (Model #Cylinder):")
    print(f"   scale: ({piston_scale_x:.3f}, {piston_scale_y:.3f}, {piston_scale_z:.3f})")
    print(f"   ⚠️  scale.y = {piston_scale_y:.3f} (очень мало!)")
    print(f"   Причина: pistonThickness=25мм → 25/100 = 0.25")
    
    if piston_scale_y < 0.5:
        print(f"   ❌ КРИТИЧНО: Поршень СЛИШКОМ ТОНКИЙ (< 0.5 единиц)!")
        print(f"   Рекомендация: Убрать деление на 100 для толщины")
    ...
    print(f"🔵 PISTON ROD (Model #Cylinder):")
    ...
    if rod_scale_x < 0.2:
        print(f"   ⚠️  Шток СЛИШКОМ ТОНКИЙ (< 0.2 единиц)!")
        print(f"   Причина: rodDiameter=35мм → (35/100)*0.5 = 0.175")
    
    if rod_scale_y < 1.0:
        print(f"   ❌ КРИТИЧНО: Шток СЛИШКОМ КОРОТКИЙ (< 1.0 единиц)!")
        print(f"   Причина: pistonRodLength=200мм → 200/100 = 2.0")
        print(f"   Рекомендация: Убрать деление на 100 для длины")
...
def calculate_optimal_scale():
    ...
    print(f"   Поршень: scale.y = {piston_thickness_mm}/{qt_cylinder_height} = {piston_scale_v1:.2f}")
    print(f"   Шток:    scale.y = {rod_length_mm}/{qt_cylinder_height} = {rod_scale_v1:.2f}")
    print()
    ...
    print(f"   Поршень: scale.y = {piston_thickness_mm}/10 = {piston_thickness_mm/10:.1f} (scale = {piston_scale_v2/100:.2f})")
    print(f"   Шток:    scale.y = {rod_length_mm}/10 = {rod_length_mm/10:.1f} (scale = {rod_scale_v2/100:.2f})")
    print()
    ...
    print(f"   Поршень: scale.y = ({piston_thickness_mm}/100) * {scene_scale_factor} = {piston_scale_v3:.2f}")
    print(f"   Шток:    scale.y = ({rod_length_mm}/100) * {scene_scale_factor} = {rod_scale_v3:.2f}")
    print()
    ...
    print(f"    scale: Qt.vector3d(1.08, pistonThickness/100, 1.08)")
    ...
    print("Model {")
    print("    source: \"#Cylinder\"")
    print(f"    scale: Qt.vector3d(0.5, pistonRodLength/100, 0.5)")
