    ...
    def check_python_version(self):
        ...
        try:
            ...
            if major < 3 or (major == 3 and minor < 8):
                ...
                print("⚠️  Python 3.13+ обнаружен. Могут быть проблемы совместимости.")
                print("📝 Рекомендуется Python 3.9-3.11 для оптимальной стабильности")
            elif major == 3 and 9 <= minor <= 11:
                print("✅ Оптимальная версия Python для проекта")
            else:
                print("✅ Подходящая версия Python")
...
"""
    ...
    def test_installation(self):
        ...
        for module_name, display_name in test_imports:
            try:
                subprocess.run([self.python_executable, '-c', f'import {module_name}'], 
        ...
        if qml_diag.exists():
            try:
                result = subprocess.run([self.python_executable, str(qml_diag)], 
        ...
        if app_file.exists():
            try:
                ...
                result = subprocess.run([self.python_executable, str(app_file), '--test-mode'], 
                ...
                print("  ⚠️  Приложение запустилось (timeout - это нормально для тестового режима)")
            except Exception as e:
