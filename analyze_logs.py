    ...
    def analyze(self) -> bool:
        ...
        try:
            ...
            for line in lines:
                ...
                # QML события
                if any(keyword in line for keyword in ['QML', 'qml:', 'apply', 'update']):
...
class LogAnalyzer:
    ...
    def run(self):
        ...
        if not self.logs_dir.exists():
            safe_print(colored("ERROR: Директория logs/ не найдена!", Colors.RED, bold=True))
            return
    ...
    def _analyze_graphics(self):
        ...
        if not graphics_dir.exists():
            safe_print(colored("ERROR: Директория logs/graphics не найдена", Colors.RED))
            return
        ...
        safe_print(f"Размер: {colored(format_size(latest.stat().st_size), Colors.CYAN)}")
        ...
        safe_print(f"\nСТАТИСТИКА:")
        safe_print(f"   Всего событий: {colored(str(stats['total']), Colors.GREEN, bold=True)}")
        synced_str = f"{stats['synced']}/{stats['total']}"
        safe_print(f"   Синхронизировано: {colored(synced_str, Colors.GREEN)} ({sync_rate:.1f}%)")
        ...
        for cat, count in sorted(stats['by_category'].items(), key=lambda x: x[1], reverse=True):
            ...
            safe_print(f"   {cat:15} {colored(str(count), Colors.CYAN):>6} ({percentage:5.1f}%)")
        ...
        for event in stats['timeline'][-5:]:
            ...
            change_str = f"{event['old']} -> {colored(str(event['new']), Colors.YELLOW)}"
    ...
    def _analyze_run(self):
        ...
        safe_print(f"Размер: {colored(format_size(run_log.stat().st_size), Colors.CYAN)}")
        ...
        safe_print(f"   Предупреждения: {colored(str(len(stats['warnings'])), Colors.YELLOW)}")
        safe_print(f"   QML события: {colored(str(len(stats['qml_events'])), Colors.CYAN)}")
        
        if stats['startup_time'] and stats['shutdown_time']:
            safe_print(f"\nВРЕМЯ СЕССИИ:")
            safe_print(f"   Запуск: {colored(stats['startup_time'], Colors.GREEN)}")
            safe_print(f"   Завершение: {colored(stats['shutdown_time'], Colors.GREEN)}")
    ...
    def _generate_summary(self):
        ...
        if self.graphics_analyzer:
            ...
            safe_print(f"   Синхронизация: {colored(f'{sync_rate:.1f}%', Colors.GREEN if sync_rate >= 90 else Colors.YELLOW)}")
        ...
        if self.ibl_analyzer:
            ...
            safe_print(f"   Ошибки: {colored(str(errors), Colors.RED if errors > 0 else Colors.GREEN)}")
        ...
        if self.run_analyzer:
            ...
            safe_print(f"\nСИСТЕМА:")
            safe_print(f"   Ошибки: {colored(str(errors), Colors.RED if errors > 0 else Colors.GREEN)}")
            safe_print(f"   Предупреждения: {colored(str(warnings), Colors.YELLOW if warnings > 0 else Colors.GREEN)}")
    ...
    def _generate_recommendations(self):
        ...
        if self.graphics_analyzer:
            ...
            if sync_rate < 95:
                recommendations.append(
                    f"Синхронизация < 95%: Проверьте QML функции applyXxxUpdates()"
                )
        ...
        if not recommendations:
            safe_print(colored("   Всё работает отлично! Проблем не обнаружено.", Colors.GREEN, bold=True))
        else:
