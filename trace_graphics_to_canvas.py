...
def trace_parameter_to_canvas(param_name: str):
    ...
    if graphics_panel.exists():
        ...
        if f"'{param_name}'" in content or f'"{param_name}"' in content:
            print(f"✅ ШАГ 1: Найден в GraphicsPanel current_graphics")
            
            # Поиск emit функции
            emit_found = False
            for emit_func in ['emit_lighting_update', 'emit_material_update', 'emit_environment_update', 
                             'emit_quality_update', 'emit_camera_update', 'emit_effects_update']:
            ...
            if not emit_found:
                print(f"❌ ШАГ 2: Параметр НЕ отправляется в QML!")
        else:
            print(f"❌ ШАГ 1: Параметр НЕ найден в GraphicsPanel!")
            return
    ...
    if main_window.exists():
        ...
        handler_found = False
        for handler in ['_on_lighting_changed', '_on_material_changed', '_on_environment_changed',
                       '_on_quality_changed', '_on_camera_changed', '_on_effects_changed']:
            if handler in content:
                ...
                if "invokeMethod" in handler_body or "updateLighting" in handler_body or \
                   "updateMaterials" in handler_body or "updateEnvironment" in handler_body or \
                   "updateQuality" in handler_body or "updateEffects" in handler_body:
                    print(f"✅ ШАГ 4: MainWindow вызывает QML функцию через {handler}")
                    handler_found = True
                    break
        
        if not handler_found:
            print(f"❌ ШАГ 4: MainWindow НЕ вызывает QML функцию!")
    ...
    if main_qml.exists():
        ...
        for prop_pattern in [f"property real {param_name}", f"property bool {param_name}", 
                            f"property int {param_name}", f"property string {param_name}",
                            f"property color {param_name}", f"property url {param_name}"]:
        ...
        update_found = False
        for update_func in ['applyLightingUpdates', 'applyMaterialUpdates', 'applyEnvironmentUpdates',
                           'applyQualityUpdates', 'applyCameraUpdates', 'applyEffectsUpdates']:
            if f"function {update_func}" in content:
                ...
                if param_name in func_body:
                    ...
                    if f"{param_name} =" in func_body:
                        print(f"✅ ШАГ 7: Параметр ПРИСВАИВАЕТСЯ в QML property")
                        update_found = True
                    else:
                        print(f"❌ ШАГ 7: Параметр НЕ присваивается!")
                    break
        
        if not update_found:
            print(f"❌ ШАГ 6: Параметр НЕ обрабатывается в update функциях!")
            return
        ...
        if "ExtendedSceneEnvironment {" in content:
            ...
            if f": {param_name}" in env_body or f": root.{param_name}" in env_body:
                print(f"✅ ШАГ 8: Параметр ПРИВЯЗАН к ExtendedSceneEnvironment!")
                canvas_found = True
                ...
                print(f"⚠️ ШАГ 8: Параметр НЕ привязан к ExtendedSceneEnvironment!")
        ...
        if "PrincipledMaterial {" in content:
            ...
            for mat_start in mat_positions:
                ...
                if f": {param_name}" in mat_body or f": root.{param_name}" in mat_body:
                    print(f"✅ ШАГ 8: Параметр ПРИВЯЗАН к PrincipledMaterial!")
                    canvas_found = True
        ...
        for light_type in ["DirectionalLight {", "PointLight {"]:
            if light_type in content:
                ...
                if f": {param_name}" in light_body or f": root.{param_name}" in light_body:
                    print(f"✅ ШАГ 8: Параметр ПРИВЯЗАН к {light_type.replace(' {', '')}!")
                    canvas_found = True
        ...
        if not canvas_found:
            print(f"❌ ШАГ 8 КРИТИЧЕСКИЙ: Параметр НЕ ИСПОЛЬЗУЕТСЯ В РЕНДЕРИНГЕ!")
            print(f"   ⚠️ Параметр есть в QML, но НЕ ПРИВЯЗАН к визуальным компонентам!")
            print(f"   💡 Нужно добавить привязку к ExtendedSceneEnvironment/PrincipledMaterial/Light")
        else:
            print(f"✅ ПОЛНАЯ ЦЕПОЧКА: Python → QML → Canvas РАБОТАЕТ!")
...
def main():
    ...
    if broken_params:
        ...
        print("=" * 80)
        print(f"Найдено {len(broken_params)} параметров, которые НЕ ВЛИЯЮТ на картинку!")
