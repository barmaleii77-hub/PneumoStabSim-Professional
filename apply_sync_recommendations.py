...
def update_qml_material_function(qml_file):
    ...
    # Заменяем функцию
    updated_content = (
        content[: match.start(2)] + new_code + "\n        " + content[match.start(2) :]
    )
...
def main():
    ...
    try:
        ...
        print(
            f"{'✅' if success_qml_materials else '❌'} Обновление applyMaterialUpdates() в QML"
        )
        print(
            f"{'✅' if success_qml_lighting else '❌'} Обновление applyLightingUpdates() в QML"
        )
        print(
            f"{'✅' if success_py_materials else '❌'} Обновление emit_material_update() в Python"
        )

        all_success = all(
            [
                success_qml_props,
                success_qml_materials,
                success_qml_lighting,
                success_py_materials,
            ]
        )
        ...
        if all_success:
            ...
            print("   py app.py --force-optimized")
        else:
