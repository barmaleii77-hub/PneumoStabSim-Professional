...
def analyze_event_pairs(events: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for i, event in enumerate(events):
        ...
        found = False
        for j in range(
            i + 1, min(i + 100, len(events))
        ):  # Ищем в следующих 100 событиях
            next_event = events[j]
            ...
            if (
                ...
                and signal_name.lower() in next_event["action"].lower()
            ):
...
def print_analysis_report(events: list[dict[str, Any]]) -> None:
    ...
    if sync_analysis["total"] > 0:
        ...
        if sync_rate < 100.0:
            print("   ⚠️  Обнаружены проблемы синхронизации!")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze Python↔QML event synchronization"
    )
